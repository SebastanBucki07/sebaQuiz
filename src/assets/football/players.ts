export const players = [
  {
    id: 1,
    name: 'Kasper Hämäläinen',
    country: 'Finlandia',
    clubs: ['TPS', 'Jablonec', 'Legia Warszawa', 'Lech Poznań', 'Djurgården'],
  },
  {
    id: 2,
    name: 'Jakub Czerwiński',
    country: 'Polska',
    clubs: [
      'Poprad Muszyna',
      'Promien Opalenica',
      'Okocimski Brzesko',
      'Bruk-Bet Termalica Nieciecza',
      'Pogoń Szczecin',
      'Legia Warszawa',
      'Piast Gliwice',
    ],
  },
  {
    id: 3,
    name: 'Michał Chrapek',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Calcio Catania', 'Lechia Gdańsk', 'Śląsk Wrocław', 'Piast Gliwice'],
  },
  {
    id: 4,
    name: 'Patryk Dziczek',
    country: 'Polska',
    clubs: ['US Salernitana 1919', 'Lazio Rzym', 'Piast Gliwice'],
  },
  {
    id: 5,
    name: 'Tom Hateley',
    country: 'Anglia',
    clubs: [
      'Basingstoke Town FC',
      'Motherwell FC',
      'Tranmere Rovers FC',
      'Śląsk Wrocław',
      'Dundee FC',
      'Piast Gliwice',
      'AEK (Larnaka)',
    ],
  },
  {
    id: 6,
    name: 'Jorge Félix',
    country: 'Hiszpania',
    clubs: [
      'Piast Gliwice',
      'Sivasspor',
      'Club Lleida Esportiu',
      'CF Rayo Majadahonda',
      'CF Trival Valderas Alcorcón',
      'AD Alcorcón B',
      'Getafe CF B',
    ],
  },
  {
    id: 7,
    name: 'Michał Kaput',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'Radomiak Radom', 'ŁKS 1926 Łomża'],
  },
  {
    id: 8,
    name: 'Damian Kądzior',
    country: 'Polska',
    clubs: [
      'Piast Gliwice',
      'Alanyaspor Kulübü',
      'SD Eibar',
      'Dinamo Zagrzeb',
      'Górnik Zabrze',
      'Wigry Suwałki',
      'Dolcan Ząbki',
      'Motor Lublin',
      'Jagiellonia Białystok',
    ],
  },
  {
    id: 9,
    name: 'Grzegorz Tomasiewicz',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'Stal Mielec', 'Pogoń Siedlce', 'Arka Gdynia'],
  },
  {
    id: 10,
    name: 'Kristopher Vida',
    country: 'Węgry',
    clubs: ['Kisvárda FC', 'Piast Gliwice', 'FC DAC 1904 Dunajská Streda', 'De Graafschap Doetinchem'],
  },
  {
    id: 11,
    name: 'Kamil Wilczek',
    country: 'Polska',
    clubs: [
      'Piast Gliwice',
      'FC Kopenhaga',
      'Göztepe',
      'Bröndby IF',
      'Carpi FC 1909',
      'Zagłębie Lubin',
      'GKS Jastrzębie',
    ],
  },
  {
    id: 12,
    name: 'Alex Sobczyk',
    country: 'Austria',
    clubs: [
      'Piast Gliwice',
      'Dóxa Katokopiás',
      'Górnik Zabrze',
      'FC Spartak Trnava',
      'Floridsdorfer AC',
      'SC Wiener Neustadt',
      'SK Niederösterreich Sankt Pölten',
      'SK Rapid Wiedeń',
      'FC Liefering',
    ],
  },
  {
    id: 13,
    name: 'Szczepan Mucha',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'Rekord Bielsko-Biała'],
  },
  {
    id: 14,
    name: 'Rauno Sappinen',
    country: 'Estonia',
    clubs: ['Stal Mielec', 'Piast Gliwice', 'FC Flora Tallin', 'NK Domžale', 'FC Den Bosch', 'KFCO Beerschot'],
  },
  {
    id: 15,
    name: 'Jakub Swierczok',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Nagoya Grampus',
      'Piast Gliwice',
      'Ludogorets Razgrad',
      'GKS Tychy',
      'Górnik Łęczna',
      'Zawisza Bydgoszcz',
      '1.FC Kaiserslautern',
      'Polonia Bytom',
    ],
  },
  {
    id: 16,
    name: 'Joel Valencia',
    country: 'Hiszpania',
    clubs: ['De Graafschap Doetinchem', 'AD Alcorcón', 'FC Brentford', 'Piast Gliwice', 'FC Koper', 'UD Logroñés'],
  },
  {
    id: 17,
    name: 'Kamil Glik',
    country: 'Polska',
    clubs: ['Benevento Calcio', 'AS Monaco', 'Torino FC', 'SSC Bari', 'Palermo FC', 'Piast Gliwice'],
  },
  {
    id: 18,
    name: 'Radoslaw Murawski',
    country: 'Polska',
    clubs: ['Lech Poznań', 'Denizlispor', 'Palermo FC', 'Piast Gliwice'],
  },
  {
    id: 19,
    name: 'Piotr Parzyszek',
    country: 'Polska',
    clubs: [
      'CD Leganés',
      'Pogoń Szczecin',
      'Frosinone Calcio',
      'Piast Gliwice',
      'PEC Zwolle',
      'De Graafschap Doetinchem',
      'Randers FC',
      'VV St. Truiden',
      'Charlton Athletic',
    ],
  },
  {
    id: 20,
    name: 'Wojciech Kędziora',
    country: 'Polska',
    clubs: [
      'Wilki Wilcza',
      'Ruch Chorzów',
      'GKS Katowice',
      'Bruk-Bet Termalica Nieciecza',
      'Piast Gliwice',
      'Zagłębie Lubin',
      'Carbo Gliwice',
    ],
  },
  {
    id: 21,
    name: 'Damian Zbozień',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Wisła Płock',
      'Arka Gdynia',
      'Zagłębie Lubin',
      'GKS Bełchatów',
      'Amkar Perm',
      'Piast Gliwice',
      'GKS Bełchatów',
      'Sandecja Nowy Sącz',
    ],
  },
  {
    id: 22,
    name: 'Marcin Robak',
    country: 'Polska',
    clubs: [
      'Widzew Łódź',
      'Śląsk Wrocław',
      'Lech Poznań',
      'Pogoń Szczecin',
      'Piast Gliwice',
      'Mersin İdman Yurdu SK',
      'Konyaspor',
      'Korona Kielce',
    ],
  },
  {
    id: 23,
    name: 'Rafał Leszczyński',
    country: 'Polska',
    clubs: ['Śląsk Wrocław', 'Chrobry Głogów', 'Podbeskidzie Bielsko-Biała', 'Piast Gliwice', 'Dolcan Ząbki'],
  },
  {
    id: 24,
    name: 'Martin Konczkowski',
    country: 'Polska',
    clubs: ['Śląsk Wrocław', 'Piast Gliwice', 'Ruch Chorzów'],
  },
  {
    id: 25,
    name: 'Dariusz Trela',
    country: 'Polska',
    clubs: [
      'Bruk-Bet Termalica Nieciecza',
      'Korona Kielce',
      'GKS Bełchatów',
      'Lechia Gdańsk',
      'Piast Gliwice',
      'Okocimski KS Brzesko',
    ],
  },
  {
    id: 26,
    name: 'Krzysztof Mączyński',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'ŁKS Łódź', 'Górnik Zabrze', 'Beijing Chengfeng', 'Legia Warszawa', 'Śląsk Wrocław'],
  },
  {
    id: 27,
    name: 'Tomasz Jodłowiec',
    country: 'Polska',
    clubs: [
      'Widzew Łódź',
      'ŁKS Łódź',
      'Podbeskidzie Bielsko-Biała',
      'Dyskobolia Grodzisk Wielkopolski',
      'Polonia Warszawa',
      'Śląsk Wrocław',
      'Legia Warszawa',
      'Piast Gliwice',
    ],
  },
  {
    id: 28,
    name: 'Bartłomiej Pawłowski',
    country: 'Polska',
    clubs: [
      'Jagiellonia Białystok',
      'GKS Katowice',
      'Warta Poznań',
      'Widzew Łódź',
      'Málaga CF',
      'Lechia Gdańsk',
      'Zawisza Bydgoszcz',
      'Korona Kielce',
      'Zagłębie Lubin',
      'Gaziantep FK',
      'Śląsk Wrocław',
    ],
  },
  {
    id: 29,
    name: 'Arkadiusz Piech',
    country: 'Polska',
    clubs: [
      'Widzew Łódź',
      'Ruch Chorzów',
      'Sivasspor',
      'Zagłębie Lubin',
      'Legia Warszawa',
      'GKS Bełchatów',
      'AEL Limassol',
      'Apollon Limassol',
      'Śląsk Wrocław',
      'Odra Opole',
      'Górnik Polkowice',
    ],
  },
  {
    id: 30,
    name: 'Djordje Čotra',
    country: 'Serbia',
    clubs: [
      'Śląsk Wrocław',
      'FK Rad Belgrad',
      'Polonia Warszawa',
      'Śląsk Wrocław',
      'FK BSK Borca',
      'FK Vojvodina Novi Sad',
      'FK Hajduk 1912',
    ],
  },
  {
    id: 31,
    name: 'Rafał Andraszak',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Flota Świnoujście', 'Piast Gliwice'],
  },
  {
    id: 32,
    name: 'Adam Banaś',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'Górnik Zabrze', 'Piast Gliwice', 'PFC Simurq', 'Ruch Radzionków'],
  },
  {
    id: 33,
    name: 'Bartosz Iwan',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'GKS Katowice', 'Piast Gliwice', 'Olimpia Elbląg', 'Widzew Łódź', 'Odra Wodzisław Śląski'],
  },
  {
    id: 34,
    name: 'Daniel Koczon',
    country: 'Polska',
    clubs: [
      'Siarka Tarnobrzeg',
      'Stal Rzeszów',
      'Motor Lublin',
      'Wisła Płock',
      'Avia Świdnik',
      'Piast Gliwice',
      'Olimpia Elbląg',
    ],
  },
  {
    id: 35,
    name: 'Adam Kompała',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Piast Gliwice', 'Górnik Zabrze'],
  },
  {
    id: 36,
    name: 'Sebastian Olszar',
    country: 'Polska',
    clubs: [
      'Greifswalder FC',
      'Flota Świnoujście',
      'Piast Gliwice',
      'Górnik Zabrze',
      'Ruch Chorzów',
      'Polonia Warszawa',
      'GKS Bełchatów',
      'Zagłębie Sosnowiec',
      'Coventry City',
      'Bruk-Bet Termalica Nieciecza',
    ],
  },
  {
    id: 37,
    name: 'Rafał Pietrzak',
    country: 'Polska',
    clubs: [
      'Lechia Gdańsk',
      'GKS Katowice',
      'Wisła Kraków',
      'Kolejarz Stróże',
      'Royal Excel Mouscron',
      'Piast Gliwice',
      'Górnik Zabrze',
      'Zagłębie Sosnowiec',
      'Zagłębie Lubin',
    ],
  },
  {
    id: 38,
    name: 'Damian Seweryn',
    country: 'Polska',
    clubs: [
      'Odra Wodzisław Śląski',
      'Widzew Łódź',
      'Górnik Zabrze',
      'BSC Young Boys',
      'Warta Poznań',
      'Piast Gliwice',
      'ŁKS Łódź',
      'Poseidon Neon Poron',
      'Polonia Warszawa',
    ],
  },
  {
    id: 39,
    name: 'Adrian Sikora',
    country: 'Polska',
    clubs: [
      'Dyskobolia Grodzisk Wielkopolski',
      'Real Murcia CF',
      'Górnik Zabrze',
      'Podbeskidzie Bielsko-Biała',
      'Piast Gliwice',
      'APOEL Nikosia',
    ],
  },
  {
    id: 40,
    name: 'Adrian Świątek',
    country: 'Polska',
    clubs: [
      'ŁKS Łódź',
      'Pelikan Łowicz',
      'Górnik Zabrze',
      'Ruch Radzionków',
      'Piast Gliwice',
      'Polonia Środa Wielkopolska',
      'Sandecja Nowy Sącz',
    ],
  },
  {
    id: 41,
    name: 'Bartosz Bereszyński',
    country: 'Polska',
    clubs: ['UC Sampdoria', 'Legia Warszawa', 'Warta Poznań', 'Lech Poznań', 'SSC Napoli'],
  },
  {
    id: 42,
    name: 'Łukasz Fabiański',
    country: 'Polska',
    clubs: ['West Ham United', 'Swansea City', 'FC Arsenal', 'Legia Warszawa', 'Lech Poznań'],
  },
  {
    id: 43,
    name: 'Maciej Murawski',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Apollon Pontou', 'Aris Saloniki', 'Lech Poznań', 'Arminia Bielefeld', 'Cracovia'],
  },
  {
    id: 44,
    name: 'Rafal Janicki',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Lech Poznań', 'Górnik Zabrze', 'Wisła Kraków', 'Podbeskidzie Bielsko-Biała'],
  },
  {
    id: 45,
    name: 'Damian Rasak',
    country: 'Polska',
    clubs: [
      'Wisła Płock',
      'Miedź Legnica',
      'Bari Primavera',
      'Chievo Primavera',
      'Elana Toruń',
      'SEF Torres 1903',
      'Górnik Zabrze',
    ],
  },
  {
    id: 46,
    name: 'Pawel Olkowski',
    country: 'Polska',
    clubs: ['Górnik Zabrze', '1.FC Köln', 'Gaziantep FK', 'Bolton Wanderers', 'GKS Katowice'],
  },
  {
    id: 47,
    name: 'Lukas Podolski',
    country: 'Polska',
    clubs: [
      'Górnik Zabrze',
      '1.FC Köln',
      'Bayern Monachium',
      'FC Arsenal',
      'Galatasaray',
      'Vissel Kobe',
      'Antalyaspor',
      'Inter Mediolan',
    ],
  },
  {
    id: 48,
    name: 'Mateusz Cholewiak',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Puszcza Niepołomice', 'Śląsk Wrocław', 'Górnik Zabrze', 'Legia Warszawa'],
  },
  {
    id: 49,
    name: 'Dominik Hladun',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'Chojniczanka Chojnice', 'Legia Warszawa'],
  },
  {
    id: 50,
    name: 'Rafal Augustyniak',
    country: 'Polska',
    clubs: [
      'Ural Jekaterynburg',
      'Miedź Legnica',
      'Pogoń Siedlce',
      'Widzew Łódź',
      'Wigry Suwałki',
      'Legia Warszawa',
      'Jagiellonia Białystok',
    ],
  },
  {
    id: 51,
    name: 'Artur Jedrzejczyk',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'FK Krasnodar', 'Dolcan Ząbki', 'GKS Jastrzębie', 'Korona Kielce'],
  },
  {
    id: 52,
    name: 'Bartosz Slisz',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Zagłębie Lubin', 'ROW 1964 Rybnik'],
  },
  {
    id: 53,
    name: 'Bartosz Kapustka',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Cracovia', 'Oud-Heverlee Leuven', 'SC Freiburg', 'Leicester City'],
  },
  {
    id: 54,
    name: 'Pawel Wszolek',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Queens Park Rangers',
      'Polonia Warszawa',
      'UC Sampdoria',
      'Hellas Verona',
      '1.FC Union Berlin',
    ],
  },
  {
    id: 55,
    name: 'Filip Mladenovic',
    country: 'Serbia',
    clubs: [
      'Legia Warszawa',
      'Lechia Gdańsk',
      'BATE Borysów',
      'Crvena Zvezda Belgrad',
      'FK Borac 1926 Cacak',
      '1.FC Köln',
      'Standard Liège',
    ],
  },
  {
    id: 56,
    name: 'Makana Baku',
    country: 'Niemcy',
    clubs: ['SG Sonnenhof Großaspach', 'Legia Warszawa', 'Göztepe', 'Holstein Kiel', 'Warta Poznań', 'SV Gonsenheim'],
  },
  {
    id: 57,
    name: 'Robert Pich',
    country: 'Słowacja',
    clubs: ['Śląsk Wrocław', 'MSK Zilina', 'Dukla Banska Bystrica', 'Legia Warszawa', '1.FC Kaiserslautern'],
  },
  {
    id: 58,
    name: 'Carlitos',
    country: 'Hiszpania',
    clubs: [
      'Legia Warszawa',
      'Panathinaikos Ateny',
      'Wisła Kraków',
      'CD Eldense',
      'Ontinyent CF',
      'CF Fuenlabrada',
      'Petrotrest St. Petersburg',
      'Al-Wahda FC Abu Dhabi',
    ],
  },
  {
    id: 59,
    name: 'Igor Sapała',
    country: 'Polska',
    clubs: ['Raków Częstochowa', 'Dolcan Ząbki', 'Wisła Kraków', 'GKS Katowice', 'Piast Gliwice'],
  },
  {
    id: 60,
    name: 'Mateusz Szczepaniak',
    country: 'Polska',
    clubs: [
      'Miedź Legnica',
      'Cracovia',
      'Podbeskidzie Bielsko-Biała',
      'Górnik Polkowice',
      'Piast Gliwice',
      'Enosis Neon Paralimniou',
      'Warta Poznań',
      'Zagłębie Lubin',
    ],
  },
  {
    id: 61,
    name: 'Jakub Kuzdra',
    country: 'Polska',
    clubs: ['Bytovia Bytów', 'Warta Poznań', 'Chrobry Głogów', 'Polonia Bytom', 'Piast Gliwice', 'Volos NPS'],
  },
  {
    id: 62,
    name: 'Karol Angielski',
    country: 'Polska',
    clubs: [
      'Radomiak Radom',
      'Olimpia Grudziądz',
      'Piast Gliwice',
      'Wisła Płock',
      'Sivasspor',
      'Zawisza Bydgoszcz',
      'Śląsk Wrocław',
      'Korona Kielce',
    ],
  },
  {
    id: 63,
    name: 'Mateusz Matras',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'Pogoń Szczecin', 'Stal Mielec', 'Górnik Zabrze', 'Zagłębie Lubin', 'Lechia Gdańsk'],
  },
  {
    id: 64,
    name: 'Piotr Malarczyk',
    country: 'Polska',
    clubs: ['Korona Kielce', 'Cracovia', 'Piast Gliwice', 'Ipswich Town', 'Southend United'],
  },
  {
    id: 65,
    name: 'Konstantin Vassiljev',
    country: 'Estonia',
    clubs: [
      'FC Flora Tallinn',
      'FCI Levadia',
      'NK Nafta 1903',
      'Jagiellonia Białystok',
      'Amkar Perm',
      'Piast Gliwice',
      'FC Koper',
      'Tallinn JK Legion',
      'HÜJK Emmaste',
    ],
  },
  {
    id: 66,
    name: 'Michal Pazdan',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Górnik Zabrze', 'Legia Warszawa', 'MKE Ankaragücü'],
  },
  {
    id: 67,
    name: 'Lukasz Sekulski',
    country: 'Polska',
    clubs: [
      'Wisła Płock',
      'ŁKS Łódź',
      'Stal Stalowa Wola',
      'Jagiellonia Białystok',
      'SKA Khabarovsk',
      'Korona Kielce',
      'Piast Gliwice',
    ],
  },
  {
    id: 68,
    name: 'Marcin Pietrowski',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Piast Gliwice', 'Miedź Legnica'],
  },
  {
    id: 69,
    name: 'Zlatan Alomerovic',
    country: 'Serbia',
    clubs: ['Jagiellonia Białystok', 'Lechia Gdańsk', 'Korona Kielce', '1.FC Kaiserslautern'],
  },
  {
    id: 70,
    name: 'Mateusz Skrzypczak',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Puszcza Niepołomice', 'Lech Poznań', 'Stomil Olsztyn'],
  },
  {
    id: 71,
    name: 'Xavier Dziekonski',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Garbarnia Kraków', 'Raków Częstochowa'],
  },
  {
    id: 72,
    name: 'Jakub Wojcicki',
    country: 'Polska',
    clubs: ['Zawisza Bydgoszcz', 'Cracovia', 'Jagiellonia Białystok', 'Zagłębie Lubin', 'Znicz Pruszków'],
  },
  {
    id: 73,
    name: 'Konrad Wrzesinski',
    country: 'Polska',
    clubs: [
      'Zagłębie Sosnowiec',
      'Pogoń Siedlce',
      'Kairat Almaty',
      'Widzew Łódź',
      'Motor Lublin',
      'Stal Mielec',
      'Jagiellonia Białystok',
      'Polonia Warszawa',
    ],
  },
  {
    id: 74,
    name: 'Rafal Gikiewicz',
    country: 'Polska',
    clubs: [
      'FC Augsburg',
      '1.FC Union Berlin',
      'Eintracht Braunschweig',
      'Śląsk Wrocław',
      'Jagiellonia Białystok',
      'SC Freiburg',
      'Stomil Olsztyn',
    ],
  },
  {
    id: 75,
    name: 'Piotr Celeban',
    country: 'Polska',
    clubs: ['Śląsk Wrocław', 'FC Vaslui', 'Pogoń Szczecin', 'Korona Kielce'],
  },
  {
    id: 76,
    name: 'Jakub Labojko',
    country: 'Polska',
    clubs: ['Brescia Calcio', 'Śląsk Wrocław', 'Raków Częstochowa', 'AEK Larnaka'],
  },
  {
    id: 77,
    name: 'Maciej Makuszewski',
    country: 'Polska',
    clubs: [
      'Lech Poznań',
      'Jagiellonia Białystok',
      'Lechia Gdańsk',
      'Odra Opole',
      'Akhmat Grozny',
      'Leiknir Reykjavík',
      'Vitoria Setúbal FC',
    ],
  },
  {
    id: 78,
    name: 'Patryk Makuch',
    country: 'Polska',
    clubs: ['Miedź Legnica', 'Cracovia', 'GKS Bełchatów'],
  },
  {
    id: 79,
    name: 'Adam Mójta',
    country: 'Polska',
    clubs: [
      'Pogoń Siedlce',
      'Sandecja Nowy Sącz',
      'Podbeskidzie Bielsko-Biała',
      'GKS Bełchatów',
      'FK Viktoria Zizkov',
      'ŁKS Łagów',
      'Wisła Kraków',
      'Warta Poznań',
      'Piast Gliwice',
      'Odra Wodzisław Śląski',
      'Miedź Legnica',
      'Korona Kielce',
    ],
  },
  {
    id: 80,
    name: 'Lukasz Skowron',
    country: 'Polska',
    clubs: [
      'Arka Gdynia',
      'AEL Limassol',
      'Jagiellonia Białystok',
      "St. Patrick's Athletic",
      'SC Ollanęs',
      'Radomiak Radom',
    ],
  },
  {
    id: 81,
    name: 'Jakub Slowik',
    country: 'Polska',
    clubs: ['Vegalta Sendai', 'Śląsk Wrocław', 'Jagiellonia Białystok', 'FC Tokyo', 'Pogoń Szczecin', 'Warta Poznań'],
  },
  {
    id: 82,
    name: 'Andrzej Niewulis',
    country: 'Polska',
    clubs: [
      'Raków Częstochowa',
      'Znicz Pruszków',
      'Miedź Legnica',
      'Ruch Radzionków',
      'Stomil Olsztyn',
      'Jagiellonia Białystok',
      'Wigry Suwałki',
    ],
  },
  {
    id: 83,
    name: 'Kamil Drygas',
    country: 'Polska',
    clubs: ['Pogoń Szczecin', 'Zawisza Bydgoszcz', 'Lech Poznań', 'Miedź Legnica'],
  },
  {
    id: 84,
    name: 'Szymon Matuszek',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Dolcan Ząbki', 'Piast Gliwice', 'Miedź Legnica', 'Jagiellonia Białystok', 'Wisła Płock'],
  },
  {
    id: 85,
    name: 'Dominik Budzynski',
    country: 'Polska',
    clubs: [
      'Radomiak Radom',
      'ŁKS Łódź',
      'Śląsk Wrocław',
      'Miedź Legnica',
      'Stomil Olsztyn',
      'Sandecja Nowy Sącz',
      'Polonia Warszawa',
    ],
  },
  {
    id: 86,
    name: 'Jonathan de Amo',
    country: 'Hiszpania',
    clubs: [
      'Stal Mielec',
      'Miedź Legnica',
      'Bruk-Bet Termalica Nieciecza',
      'Górnik Łęczna',
      'Widzew Łódź',
      'Ontinyent CF',
      'UE Sant Andreu',
    ],
  },
  {
    id: 87,
    name: 'Bartosz Kopacz',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Górnik Zabrze',
      'Bruk-Bet Termalica Nieciecza',
      'Lechia Gdańsk',
      'Ruch Radzionków',
      'Zawisza Bydgoszcz',
    ],
  },
  {
    id: 88,
    name: 'Mateusz Grzybek',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'GKS Tychy', 'Bruk-Bet Termalica Nieciecza', 'Radomiak Radom'],
  },
  {
    id: 89,
    name: 'Maciej Dabrowski',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'ŁKS Łódź',
      'Pogoń Szczecin',
      'Legia Warszawa',
      'Olimpia Grudziądz',
      'Zawisza Bydgoszcz',
      'GKS Bełchatów',
    ],
  },
  {
    id: 90,
    name: 'Wojciech Trochim',
    country: 'Polska',
    clubs: [
      'Sandecja Nowy Sącz',
      'Kolejarz Stróże',
      'Pogoń Siedlce',
      'Chojniczanka Chojnice',
      'Dolcan Ząbki',
      'GKS Katowice',
      'KSZO Ostrowiec Świętokrzyski',
      'Warta Poznań',
      'KS Wiązownica',
      'GKS Tychy',
      'Podbeskidzie Bielsko-Biała',
      'Bałtyk Gdynia',
      'Zagłębie Lubin',
      'Legia Warszawa',
    ],
  },
  {
    id: 91,
    name: 'Michal Gliwa',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Raków Częstochowa',
      'Sandecja Nowy Sącz',
      'Zagłębie Sosnowiec',
      'Polonia Warszawa',
      'Pandurii Targu Jiu',
      'Stal Mielec',
      'Dyskobolia Grodzisk Wielkopolski',
    ],
  },
  {
    id: 92,
    name: 'Krzysztof Piatek',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Cracovia',
      'Hertha BSC',
      'AC Milan',
      'US Salernitana 1919',
      'Genoa CFC',
      'AC Fiorentina',
    ],
  },
  {
    id: 93,
    name: 'Manuel Arboleda',
    country: 'Kolumbia',
    clubs: ['Zagłębie Lubin', 'Lech Poznań', 'Club Cienciano'],
  },
  {
    id: 94,
    name: 'Patryk Tuszynski',
    country: 'Polska',
    clubs: [
      'MKS Kluczbork',
      'Zagłębie Lubin',
      'Caykur Rizespor',
      'Wisła Płock',
      'Jagiellonia Białystok',
      'Lechia Gdańsk',
      'Piast Gliwice',
      'Chojniczanka Chojnice',
      'Sandecja Nowy Sącz',
    ],
  },
  {
    id: 95,
    name: 'Bartosz Rymaniak',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'Korona Kielce', 'Piast Gliwice', 'Cracovia', 'Górnik Łęczna', 'Arka Gdynia'],
  },
  {
    id: 96,
    name: 'Patryk Mucha',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'Widzew Łódź', 'Chrobry Głogów', 'Górnik Polkowice'],
  },
  {
    id: 97,
    name: 'Pawel Sasin',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Korona Kielce',
      'Lech Poznań',
      'Cracovia',
      'Polonia Nysa',
      'ŁKS Łódź',
      'Warta Poznań',
      'Arka Gdynia',
      'Dolcan Ząbki',
      'Śląsk Wrocław',
    ],
  },
  {
    id: 98,
    name: 'Jacek Kielb',
    country: 'Polska',
    clubs: ['Korona Kielce', 'Lech Poznań', 'Bruk-Bet Termalica Nieciecza', 'Śląsk Wrocław', 'Polonia Warszawa'],
  },
  {
    id: 99,
    name: 'Jakub Wrabel',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'Śląsk Wrocław', 'Olimpia Grudziądz', 'Stal Mielec', 'Wisła Płock'],
  },
  {
    id: 100,
    name: 'Mateusz Radecki',
    country: 'Polska',
    clubs: ['Radomiak Radom', 'Śląsk Wrocław', 'Wigry Suwałki', 'GKS Tychy'],
  },
  {
    id: 101,
    name: 'Mateusz Kochalski',
    country: 'Polska',
    clubs: ['Radomiak Radom', 'Legionovia Legionowo', 'Stal Mielec'],
  },
  {
    id: 102,
    name: 'Michal Grudniewski',
    country: 'Polska',
    clubs: [
      'Radomiak Radom',
      'Legionovia Legionowo',
      'Widzew Łódź',
      'Zagłębie Sosnowiec',
      'Polonia Warszawa',
      'Pogoń Siedlce',
    ],
  },
  {
    id: 103,
    name: 'Maciej Gorski',
    country: 'Polska',
    clubs: [
      'Pogoń Siedlce',
      'Korona Kielce',
      'Znicz Pruszków',
      'Sandecja Nowy Sącz',
      'Resovia Rzeszów',
      'Radomiak Radom',
      'Chrobry Głogów',
      'Stilon Gorzów Wielkopolski',
      'Chojniczanka Chojnice',
      'Jagiellonia Białystok',
      'Arka Gdynia',
      'Legia Warszawa',
    ],
  },
  {
    id: 104,
    name: 'Mateusz Lewandowski',
    country: 'Polska',
    clubs: ['Pogoń Szczecin', 'Radomiak Radom', 'Śląsk Wrocław', 'Lechia Gdańsk', 'Virtus Entella'],
  },
  {
    id: 105,
    name: 'Mateusz Bodzioch',
    country: 'Polska',
    clubs: [
      'Odra Opole',
      'Zagłębie Sosnowiec',
      'ROW 1964 Rybnik',
      'Stal Mielec',
      'Piast Gliwice',
      'Radomiak Radom',
      'Ruch Zdzieszowice',
      'Warta Poznań',
      'Korona Kielce',
    ],
  },
  {
    id: 106,
    name: 'Meik Karwot',
    country: 'Polska',
    clubs: [
      'Radomiak Radom',
      'VfR Wormatia Worms',
      'Górnik Zabrze',
      'Zagłębie Sosnowiec',
      'Pogoń Siedlce',
      'SVN Zweibrücken',
      'Lüneburger SK Hansa',
      'APO Fostiras',
      'FC Nordkirchen',
    ],
  },
  {
    id: 107,
    name: 'Grzegorz Bonin',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Górnik Zabrze',
      'Korona Kielce',
      'Chełmianka Chełm',
      'Motor Lublin',
      'Polonia Warszawa',
      'ŁKS Łódź',
      'Pogoń Szczecin',
    ],
  },
  {
    id: 108,
    name: 'Szymon Drewniak',
    country: 'Polska',
    clubs: [
      'Chrobry Głogów',
      'Górnik Łęczna',
      'Lech Poznań',
      'Chojniczanka Chojnice',
      'Cracovia',
      'Arka Gdynia',
      'Odra Opole',
      'Górnik Zabrze',
    ],
  },
  {
    id: 109,
    name: 'Lukasz Mierzejewski',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Zagłębie Lubin',
      'Cracovia',
      'Widzew Łódź',
      'Świt Nowy Dwór Mazowiecki',
      'HNK Rijeka',
      'Legia Warszawa',
      'Podbeskidzie Bielsko-Biała',
    ],
  },
  {
    id: 110,
    name: 'Hubert Adamczyk',
    country: 'Polska',
    clubs: ['Arka Gdynia', 'GKS Tychy', 'Wisła Płock', 'Olimpia Grudziądz', 'Cracovia'],
  },
  {
    id: 111,
    name: 'Pawel Nowak',
    country: 'Polska',
    clubs: ['Cracovia', 'Lechia Gdańsk', 'Wisła Kraków', 'Garbarnia Kraków', 'Sandecja Nowy Sącz'],
  },
  {
    id: 112,
    name: 'Cornel Rapa',
    country: 'Rumunia',
    clubs: ['Cracovia', 'SC Otelul Galati', 'FCSB', 'Pogoń Szczecin'],
  },
  {
    id: 113,
    name: 'Radoslaw Majewski',
    country: 'Polska',
    clubs: [
      'Nottingham Forest',
      'Lech Poznań',
      'Dyskobolia Grodzisk Wielkopolski',
      'Polonia Warszawa',
      'Pogoń Szczecin',
      'Wieczysta Kraków',
      'Veria NPS',
      'Huddersfield Town',
      'Western Sydney Wanderers',
    ],
  },
  {
    id: 114,
    name: 'Wojciech Lisowski',
    country: 'Polska',
    clubs: ['Chojniczanka Chojnice', 'GKS Katowice', 'Stal Mielec', 'Piast Gliwice', 'Pogoń Szczecin'],
  },
  {
    id: 115,
    name: 'Lukasz Budzilek',
    country: 'Polska',
    clubs: [
      'GKS Katowice',
      'Bruk-Bet Termalica Nieciecza',
      'Chojniczanka Chojnice',
      'Motor Lublin',
      'Wigry Suwałki',
      'Lechia Gdańsk',
      'GKS Bełchatów',
      'Pogoń Szczecin',
    ],
  },
  {
    id: 116,
    name: 'Bartosz Fabiniak',
    country: 'Polska',
    clubs: ['Olimpia Grudziądz', 'Widzew Łódź', 'Pogoń Szczecin'],
  },
  {
    id: 117,
    name: 'Bartosz Nowak',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Górnik Zabrze', 'Ruch Chorzów', 'Raków Częstochowa', 'Polonia Bytom', 'Miedź Legnica'],
  },
  {
    id: 118,
    name: 'Jean Carlos Silva',
    country: 'Hiszpania',
    clubs: [
      'Recreativo Granada',
      'Pogoń Szczecin',
      'Wisła Kraków',
      'Raków Częstochowa',
      'CF Fuenlabrada',
      'Granada CF',
    ],
  },
  {
    id: 119,
    name: 'Miguel Luís',
    country: 'Portugal',
    clubs: ['Warta Poznań', 'Vitoria Guimarães SC', 'Sporting CP', 'Raków Częstochowa'],
  },
  {
    id: 120,
    name: 'Daniel Szelagowski',
    country: 'Polska',
    clubs: ['Raków Częstochowa', 'Korona Kielce', 'Warta Poznań', 'Chojniczanka Chojnice'],
  },
  {
    id: 121,
    name: 'Milosz Szczepanski',
    country: 'Polska',
    clubs: ['Raków Częstochowa', 'Warta Poznań', 'Lechia Gdańsk', 'Legia Warszawa'],
  },
  {
    id: 122,
    name: 'Valeriane Gvilia',
    country: 'Gruzja',
    clubs: [
      'Legia Warszawa',
      'BATE Borysów',
      'Metalurg Zaporizhya',
      'Raków Częstochowa',
      'FC Luzern',
      'FK Minsk',
      'Górnik Zabrze',
    ],
  },
  {
    id: 123,
    name: 'Oskar Zawada',
    country: 'Polska',
    clubs: [
      'Wisła Płock',
      'Wellington Phoenix',
      'Arka Gdynia',
      'FC Twente Enschede',
      'Stal Mielec',
      'Jeju United',
      'Raków Częstochowa',
    ],
  },
  {
    id: 124,
    name: 'Dariusz Formella',
    country: 'Polska',
    clubs: [
      'Lech Poznań',
      'Sacramento Republic FC',
      'Oakland Roots SC',
      'Arka Gdynia',
      'Raków Częstochowa',
      'Pogoń Szczecin',
    ],
  },
  {
    id: 125,
    name: 'Peter Mazan',
    country: 'Słowacja',
    clubs: [
      'FK Pohronie',
      'AS Trencin',
      'ViOn Zlate Moravce-Vrable',
      'Radomiak Radom',
      'MFK Skalica',
      'Raków Częstochowa',
      'MFK Dukla Banska Bystrica',
    ],
  },
  {
    id: 126,
    name: 'Michael Ameyaw',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'Piast Gliwice'],
  },
  {
    id: 127,
    name: 'Tomasz Mokwa',
    country: 'Polska',
    clubs: ['Calisia Kalisz', 'Piast Gliwice', 'Flota Świnoujście', 'GKS Katowice'],
  },
  {
    id: 128,
    name: 'Karol Szymanski',
    country: 'Polska',
    clubs: [
      'Jarota Jarocin',
      'Polonia Środa Wielkopolska',
      'Chrobry Głogów',
      'Lech Rypin',
      'Kotwica Kołobrzeg',
      'Chojniczanka Chojnice',
      'Piast Gliwice',
      'Lech Poznań',
      'Ostrovia 1909 Ostrów Wielkopolski',
    ],
  },
  {
    id: 129,
    name: 'Zoran Arsenic',
    country: 'Chorwacja',
    clubs: [
      'Raków Częstochowa',
      'NK Osijek',
      'Jagiellonia Białystok',
      'Wisła Kraków',
      'NK Sesvete',
      'HNK Segesta Sisak',
      'HNK Rijeka',
    ],
  },
  {
    id: 130,
    name: 'Szymon Czyz',
    country: 'Polska',
    clubs: ['Raków Częstochowa', 'Warta Poznań', 'Lazio Rzym'],
  },
  {
    id: 131,
    name: 'Fabian Piasecki',
    country: 'Polska',
    clubs: [
      'Miedź Legnica',
      'Śląsk Wrocław',
      'Zagłębie Sosnowiec',
      'Olimpia Zambrów',
      'Raków Częstochowa',
      'Stal Mielec',
      'Rozwój Katowice',
      'Górnik Zabrze',
    ],
  },
  {
    id: 132,
    name: 'Sebastian Musiolik',
    country: 'Polska',
    clubs: ['Raków Częstochowa', 'ROW 1964 Rybnik', 'Pordenone Calcio', 'Piast Gliwice'],
  },
  {
    id: 133,
    name: 'Mateusz Wdowiak',
    country: 'Polska',
    clubs: ['Cracovia', 'Raków Częstochowa', 'Sandecja Nowy Sącz'],
  },
  {
    id: 134,
    name: 'Marcin Cebula',
    country: 'Polska',
    clubs: ['Korona Kielce', 'Raków Częstochowa'],
  },
  {
    id: 135,
    name: 'Cezary Miszta',
    country: 'Polska',
    clubs: ['Legia Warszawa', ' Radomiak Radom'],
  },
  {
    id: 136,
    name: 'Maik Nawrocki',
    country: 'Polska',
    clubs: ['Legia Warszawa', ' Warta Poznań'],
  },
  {
    id: 137,
    name: 'Patryk Sokolowski',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'Olimpia Elbląg', 'Legia Warszawa', 'Wigry Suwałki', 'Znicz Pruszków'],
  },
  {
    id: 138,
    name: 'Bartosz Salamon',
    country: 'Polska',
    clubs: [
      'Brescia Calcio',
      'Lech Poznań',
      'Cagliari Calcio',
      'SPAL',
      'Delfino Pescara 1936',
      'Calcio Foggia 1920',
      'Frosinone Calcio',
      'UC Sampdoria',
    ],
  },
  {
    id: 139,
    name: 'Alan Czerwinski',
    country: 'Polska',
    clubs: ['GKS Katowice', 'Zagłębie Lubin', 'Lech Poznań'],
  },
  {
    id: 140,
    name: 'Artur Sobiech',
    country: 'Polska',
    clubs: [
      'Hannover 96',
      'Ruch Chorzów',
      'Lechia Gdańsk',
      'Fatih Karagümrük',
      'Lech Poznań',
      'Polonia Warszawa',
      'SV Darmstadt 98',
    ],
  },
  {
    id: 141,
    name: 'Mariusz Malec',
    country: 'Polska',
    clubs: ['Pogoń Szczecin', 'Podbeskidzie Bielsko-Biała', 'Olimpia Grudziądz', 'Polonia Bytom'],
  },
  {
    id: 142,
    name: 'Pawel Stolarski',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Legia Warszawa', 'Pogoń Szczecin', 'Wisła Kraków', 'Zagłębie Lubin'],
  },
  {
    id: 143,
    name: 'Rafal Kurzawa',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Pogoń Szczecin', 'ROW 1964 Rybnik', 'Amiens SC', 'Esbjerg fB', 'FC Midtjylland'],
  },
  {
    id: 144,
    name: 'Michal Kucharczyk',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Pogoń Szczecin', 'Ural Jekaterynburg'],
  },
  {
    id: 145,
    name: 'Szymon Wlodarczyk',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Legia Warszawa'],
  },
  {
    id: 146,
    name: 'Jaroslaw Jach',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Raków Częstochowa',
      'FC Sheriff Tyraspol',
      'Caykur Rizespor',
      'Fortuna Sittard',
      'Crystal Palace',
    ],
  },
  {
    id: 147,
    name: 'Marcin Flis',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Sandecja Nowy Sącz', 'GKS Bełchatów', 'Górnik Łęczna', 'GKS Katowice', 'Piast Gliwice'],
  },
  {
    id: 148,
    name: 'Piotr Wlazlo',
    country: 'Polska',
    clubs: ['Bruk-Bet Termalica Nieciecza', 'Wisła Płock', 'Stal Mielec', 'Radomiak Radom', 'Jagiellonia Białystok'],
  },
  {
    id: 149,
    name: 'Fabian Hiszpanski',
    country: 'Polska',
    clubs: [
      'Arka Gdynia',
      'Wisła Płock',
      'Stal Mielec',
      'KKS 1925 Kalisz',
      'Wisła Puławy',
      'Bytovia Bytów',
      'Podbeskidzie Bielsko-Biała',
    ],
  },
  {
    id: 150,
    name: 'Krystian Getinger',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Stal Stalowa Wola', 'Zagłębie Lubin'],
  },
  {
    id: 151,
    name: 'Mateusz Mak',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Piast Gliwice', 'GKS Bełchatów', 'Ruch Radzionków'],
  },
  {
    id: 152,
    name: 'Patryk Lipski',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Piast Gliwice', 'Ruch Chorzów', 'Widzew Łódź'],
  },
  {
    id: 153,
    name: 'Mateusz Zyro',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Widzew Łódź', 'Wigry Suwałki', 'Miedź Legnica', 'Legia Warszawa'],
  },
  {
    id: 154,
    name: 'Ernest Terpilowski',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'Górnik Polkowice', 'Lechia Dzierzoniow', 'Bruk-Bet Termalica Nieciecza'],
  },
  {
    id: 155,
    name: 'Lukasz Zjawinski',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Widzew Łódź', 'Stal Stalowa Wola', 'Sandecja Nowy Sącz', 'Lechia Gdańsk'],
  },
  {
    id: 156,
    name: 'Bartosz Kwiecien',
    country: 'Polska',
    clubs: [
      'Jagiellonia Białystok',
      'Korona Kielce',
      'Resovia Rzeszów',
      'Chrobry Głogów',
      'Arka Gdynia',
      'Górnik Łęczna',
    ],
  },
  {
    id: 157,
    name: 'Adam Deja',
    country: 'Polska',
    clubs: ['Arka Gdynia', 'Podbeskidzie Bielsko-Biała', 'Korona Kielce', 'MKS Kluczbork', 'Cracovia'],
  },
  {
    id: 158,
    name: 'Jakub Rzezniczak',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Wisła Płock', 'Qarabağ FK', 'Widzew Łódź'],
  },
  {
    id: 159,
    name: 'Bartosz Spiaczka',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Flota Świnoujście',
      'Bruk-Bet Termalica Nieciecza',
      'GKS Katowice',
      'Podbeskidzie Bielsko-Biała',
      'Korona Kielce',
      'Wisła Płock',
    ],
  },
  {
    id: 160,
    name: 'Rafal Wolski',
    country: 'Polska',
    clubs: [
      'Lechia Gdańsk',
      'Wisła Płock',
      'Legia Warszawa',
      'KV Mechelen',
      'AC Fiorentina',
      'Wisła Kraków',
      'SSC Bari',
    ],
  },
  {
    id: 161,
    name: 'Mateusz Szwoch',
    country: 'Polska',
    clubs: ['Arka Gdynia', 'Wisła Płock', 'Legia Warszawa'],
  },
  {
    id: 162,
    name: 'Dominik Furman',
    country: 'Polska',
    clubs: ['Wisła Płock', 'Legia Warszawa', 'Genclerbirligi Ankara', 'FC Toulouse', 'Hellas Verona'],
  },
  {
    id: 163,
    name: 'Dusan Kuciak',
    country: 'Słowacja',
    clubs: ['Lechia Gdańsk', 'Legia Warszawa', 'FC Vaslui', 'MSK Zilina', 'Hull City'],
  },
  {
    id: 164,
    name: 'Michal Nalepa',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Ferencvárosi TC', 'Bruk-Bet Termalica Nieciecza', 'Ruch Radzionków', 'Wisła Kraków'],
  },
  {
    id: 165,
    name: 'Maciej Gajos',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'Lech Poznań', 'Jagiellonia Białystok'],
  },
  {
    id: 166,
    name: 'Lukasz Zwolinski',
    country: 'Polska',
    clubs: ['Pogoń Szczecin', 'Lechia Gdańsk', 'HNK Gorica', 'Górnik Łęczna', 'Śląsk Wrocław', 'Arka Gdynia'],
  },
  {
    id: 167,
    name: 'Flávio Paixão',
    country: 'Portugalia',
    clubs: ['Tractor FC', 'Lechia Gdańsk', 'Hamilton Academical FC', 'Benidorm CF', 'Śląsk Wrocław', 'CF Villanovense'],
  },
  {
    id: 168,
    name: 'Sebastian Milewski',
    country: 'Polska',
    clubs: ['Zagłębie Sosnowiec', 'Piast Gliwice', 'Arka Gdynia', 'Legionovia Legionowo'],
  },
  {
    id: 169,
    name: 'Janusz Gol',
    country: 'Polska',
    clubs: [
      'Amkar Perm',
      'Legia Warszawa',
      'GKS Bełchatów',
      'Cracovia',
      'Górnik Łęczna',
      'Arka Gdynia',
      'FC Dinamo 1948',
    ],
  },
  {
    id: 170,
    name: 'Omran Haydary',
    country: 'Afganistan',
    clubs: ['Arka Gdynia', 'Lechia Gdańsk', 'Olimpia Grudziądz', 'FC Dordrecht', 'FC Emmen'],
  },
  {
    id: 171,
    name: 'Bartosz Szeliga',
    country: 'Polska',
    clubs: ['Piast Gliwice', 'ŁKS Łódź', 'GKS Tychy', 'Bruk-Bet Termalica Nieciecza', 'Sandecja Nowy Sącz'],
  },
  {
    id: 172,
    name: 'Maciej Gostomski',
    country: 'Polska',
    clubs: [
      'Górnik Łęczna',
      'Lech Poznań',
      'Bytovia Bytów',
      'Korona Kielce',
      'Cracovia',
      'Chojniczanka Chojnice',
      'FK Haugesund',
    ],
  },
  {
    id: 173,
    name: 'Igor Lasicki',
    country: 'Polska',
    clubs: ['Wisła Płock', 'Wisła Kraków', 'AS Gubbio 1910', 'Pogoń Szczecin', 'Rimini FC', 'AC Carpi', 'SSC Napoli'],
  },
  {
    id: 174,
    name: 'Jakub Blaszczykowski',
    country: 'Polska',
    clubs: ['Borussia Dortmund', 'Wisła Kraków', 'VfL Wolfsburg', 'AC Fiorentina'],
  },
  {
    id: 175,
    name: 'Michal Zyro',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Piast Gliwice',
      'Wisła Kraków',
      'Stal Mielec',
      'Charlton Athletic',
      'Korona Kielce',
      'Jagiellonia Białystok',
      'Pogoń Szczecin',
      'Wolverhampton Wanderers',
    ],
  },
  {
    id: 176,
    name: 'Alan Uryga',
    country: 'Polska',
    clubs: ['Wisła Płock', 'Wisła Kraków'],
  },
  {
    id: 177,
    name: 'Karol Linetty',
    country: 'Polska',
    clubs: ['UC Sampdoria', 'Lech Poznań', 'Torino FC'],
  },
  {
    id: 178,
    name: 'Lukasz Skorupski',
    country: 'Polska',
    clubs: ['FC Bologna', 'FC Empoli', 'Górnik Zabrze', 'AS Roma', 'Ruch Radzionków'],
  },
  {
    id: 179,
    name: 'Bartlomiej Dragowski',
    country: 'Polska',
    clubs: ['AC Fiorentina', 'Jagiellonia Białystok', 'Spezia Calcio', 'FC Empoli'],
  },
  {
    id: 180,
    name: 'Wojciech Szczesny',
    country: 'Polska',
    clubs: ['Juventus Turyn', 'FC Arsenal', 'AS Roma', 'FC Brentford'],
  },
  {
    id: 181,
    name: 'Jakub Kiwior',
    country: 'Polska',
    clubs: ['MSK Zilina', 'Spezia Calcio', 'FK Zeleziarne Podbrezova', 'FC Arsenal'],
  },
  {
    id: 182,
    name: 'Jan Bednarek',
    country: 'Polska',
    clubs: ['FC Southampton', 'Lech Poznań', 'Górnik Łęczna', 'Aston Villa'],
  },
  {
    id: 183,
    name: 'Mateusz Wieteska',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Górnik Zabrze', 'Clermont Foot 63', 'Chrobry Głogów', 'Dolcan Ząbki'],
  },
  {
    id: 184,
    name: 'Przemyslaw Wisniewski',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Venezia FC', 'Spezia Calcio'],
  },
  {
    id: 185,
    name: 'Arkadiusz Reca',
    country: 'Polska',
    clubs: ['Wisła Płock', 'FC Crotone', 'Spezia Calcio', 'Flota Świnoujście', 'SPAL'],
  },
  {
    id: 186,
    name: 'Tomasz Kedziora',
    country: 'Polska',
    clubs: ['Dynamo Kijów', 'Lech Poznań', 'PAOK Saloniki'],
  },
  {
    id: 187,
    name: 'Krystian Bielik',
    country: 'Polska',
    clubs: ['Derby County', 'Birmingham City', 'Charlton Athletic', 'Legia Warszawa', 'FC Arsenal'],
  },
  {
    id: 188,
    name: 'Damian Szymanski',
    country: 'Polska',
    clubs: ['AEK Ateny', 'Wisła Płock', 'GKS Bełchatów', 'Jagiellonia Białystok', 'Akhmat Grozny'],
  },
  {
    id: 189,
    name: 'Piotr Zielinski',
    country: 'Polska',
    clubs: ['SSC Napoli', 'FC Empoli', 'Udinese Calcio'],
  },
  {
    id: 190,
    name: 'Przemyslaw Frankowski',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'RC Lens', 'Chicago Fire FC', 'Lechia Gdańsk'],
  },
  {
    id: 191,
    name: 'Jakub Kaminski',
    country: 'Polska',
    clubs: ['Lech Poznań', 'VfL Wolfsburg'],
  },
  {
    id: 192,
    name: 'Robert Lewandowski',
    country: 'Polska',
    clubs: ['Bayern Monachium', 'Borussia Dortmund', 'Lech Poznań', 'FC Barcelona', 'Znicz Pruszków'],
  },
  {
    id: 193,
    name: 'Arkadiusz Milik',
    country: 'Polska',
    clubs: [
      'SSC Napoli',
      'Ajax Amsterdam',
      'Olympique Marseille',
      'Górnik Zabrze',
      'Juventus Turyn',
      'FC Augsburg',
      'Bayer 04 Leverkusen',
    ],
  },
  {
    id: 194,
    name: 'Karol Swiderski',
    country: 'Polska',
    clubs: ['PAOK Saloniki', 'Jagiellonia Białystok', 'Charlotte FC'],
  },
  {
    id: 195,
    name: 'Kamil Grabara',
    country: 'Polska',
    clubs: ['FC Kopenhaga', 'Aarhus GF', 'Huddersfield Town'],
  },
  {
    id: 196,
    name: 'Michal Helik',
    country: 'Polska',
    clubs: ['Cracovia', 'FC Barnsley', 'Ruch Chorzów', 'Huddersfield Town'],
  },
  {
    id: 197,
    name: 'Marcin Kaminski',
    country: 'Polska',
    clubs: ['Lech Poznań', 'VfB Stuttgart', 'FC Schalke 04', 'Fortuna Düsseldorf'],
  },
  {
    id: 198,
    name: 'Matty Cash',
    country: 'Polska',
    clubs: ['Nottingham Forest', 'Aston Villa', 'FC Dagenham & Redbridge'],
  },
  {
    id: 199,
    name: 'Kamil Józwiak',
    country: 'Polska',
    clubs: ['Lech Poznań', 'Derby County', 'Charlotte FC', 'GKS Katowice'],
  },
  {
    id: 200,
    name: 'Przemyslaw Placheta',
    country: 'Polska',
    clubs: [
      'Norwich City',
      'Śląsk Wrocław',
      'Podbeskidzie Bielsko-Biała',
      'Pogoń Siedlce',
      'Birmingham City',
      'SG Sonnenhof Großaspach',
    ],
  },
  {
    id: 201,
    name: 'Kamil Grosicki',
    country: 'Polska',
    clubs: [
      'Hull City',
      'Sivasspor',
      'Stade Rennais FC',
      'Pogoń Szczecin',
      'Jagiellonia Białystok',
      'West Bromwich Albion',
      'Legia Warszawa',
      'FC Sion',
    ],
  },
  {
    id: 202,
    name: 'Radoslaw Majecki',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Cercle Brügge', 'Stal Mielec', 'AS Monaco'],
  },
  {
    id: 203,
    name: 'Pawel Dawidowicz',
    country: 'Polska',
    clubs: ['Hellas Verona', 'Lechia Gdańsk', 'Palermo FC', 'VfL Bochum'],
  },
  {
    id: 204,
    name: 'Maciej Rybus',
    country: 'Polska',
    clubs: ['Lokomotiw Moskwa', 'Legia Warszawa', 'Akhmat Grozny', 'Olympique Lyon', 'Spartak Moskwa'],
  },
  {
    id: 205,
    name: 'Grzegorz Krychowiak',
    country: 'Polska',
    clubs: [
      'Stade Reims',
      'Lokomotiw Moskwa',
      'Sevilla FC',
      'Al-Shabab FC',
      'West Bromwich Albion',
      'FC Nantes',
      'FC Paris Saint-Germain',
      'FK Krasnodar',
      'AEK Ateny',
      'FC Girondins Bordeaux',
    ],
  },
  {
    id: 206,
    name: 'Mateusz Klich',
    country: 'Polska',
    clubs: [
      'Leeds United',
      'Cracovia',
      'PEC Zwolle',
      'FC Twente Enschede',
      '1.FC Kaiserslautern',
      'D.C. United',
      'FC Utrecht',
    ],
  },
  {
    id: 207,
    name: 'Thiago Cionek',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Reggina 1914', 'Modena FC', 'SPAL', 'Palermo FC', 'Calcio Padova'],
  },
  {
    id: 208,
    name: 'Lukasz Piszczek',
    country: 'Polska',
    clubs: ['Borussia Dortmund', 'Hertha BSC', 'Zagłębie Lubin', 'LKS Goczałkowice-Zdrój'],
  },
  {
    id: 209,
    name: 'Dawid Kownacki',
    country: 'Polska',
    clubs: ['Lech Poznań', 'Fortuna Düsseldorf', 'UC Sampdoria'],
  },
  {
    id: 210,
    name: 'Bartosz Bialkowski',
    country: 'Polska',
    clubs: ['Ipswich Town', 'FC Millwall', 'Notts County', 'FC Southampton', 'Górnik Zabrze', 'FC Barnsley'],
  },
  {
    id: 211,
    name: 'Artur Boruc',
    country: 'Polska',
    clubs: ['Celtic Glasgow', 'Legia Warszawa', 'AFC Bournemouth', 'AC Fiorentina', 'FC Southampton'],
  },
  {
    id: 212,
    name: 'Igor Lewczuk',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Znicz Pruszków',
      'Jagiellonia Białystok',
      'FC Girondins Bordeaux',
      'Ruch Chorzów',
      'Zawisza Bydgoszcz',
      'Piast Gliwice',
    ],
  },
  {
    id: 213,
    name: 'Lukasz Teodorczyk',
    country: 'Polska',
    clubs: [
      'RSC Anderlecht',
      'Lech Poznań',
      'Dynamo Kijów',
      'Polonia Warszawa',
      'Udinese Calcio',
      'RSC Charleroi',
      'LR Vicenza',
    ],
  },
  {
    id: 214,
    name: 'Przemyslaw Tyton',
    country: 'Polska',
    clubs: [
      'FC Cincinnati',
      'PSV Eindhoven',
      'Elche CF',
      'VfB Stuttgart',
      'Górnik Łęczna',
      'Deportivo de La Coruña',
      'FC Twente Enschede',
    ],
  },
  {
    id: 215,
    name: 'Lukasz Szukala',
    country: 'Polska',
    clubs: [
      'FCSB',
      'TSV 1860 München',
      'Alemannia Aachen',
      'FC Universitatea Cluj',
      'Al-Ittihad Club',
      'Ankaraspor',
      'MKE Ankaragücü',
      'Petrolul Ploiesti',
    ],
  },
  {
    id: 216,
    name: 'Jakub Wawrzyniak',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Lechia Gdańsk',
      'Widzew Łódź',
      'GKS Katowice',
      'Amkar Perm',
      'Panathinaikos Ateny',
      'Świt Nowy Dwór Mazowiecki',
    ],
  },
  {
    id: 217,
    name: 'Slawomir Peszko',
    country: 'Polska',
    clubs: [
      'Wisła Płock',
      'Lechia Gdańsk',
      'Lech Poznań',
      '1.FC Köln',
      'Wolverhampton Wanderers',
      'Wisła Kraków',
      'Wieczysta Kraków',
    ],
  },
  {
    id: 218,
    name: "N'Golo Kanté",
    country: 'Francja',
    clubs: ['Chelsea FC', 'SM Caen', 'Leicester City', 'US Boulogne'],
  },
  {
    id: 219,
    name: 'Jesus imaz',
    country: 'Hiszpania',
    clubs: [
      'Jagiellonia Białystok',
      'Lleida Esportiu',
      'UE Costa Brava',
      'Wisła Kraków',
      'Unió Esportiva Lleida',
      'UCAM Murcia CF',
      'Cádiz CF',
    ],
  },
  {
    id: 220,
    name: 'Tomasz Kupisz',
    country: 'Polska',
    clubs: [
      'Jagiellonia Białystok',
      'Brescia Calcio',
      'Cesena FC',
      'US Salernitana 1919',
      'Novara FC',
      'AS Cittadella',
      'FC Trapani 1905',
      'Reggina 1914',
      'SSC Bari',
      'Pordenone Calcio',
      'Ascoli Calcio',
      'US Livorno 1915',
      'Oldham Athletic',
      'Wigan Athletic',
      'Chievo Verona',
    ],
  },
  {
    id: 221,
    name: 'Bartosz Bida',
    country: 'Polska',
    clubs: ['Jagiellonia Białystok', 'Wigry Suwałki'],
  },
  {
    id: 222,
    name: 'Lukasz Surma',
    country: 'Polska',
    clubs: ['Ruch Chorzów', 'Legia Warszawa', 'Lechia Gdańsk', 'Wisła Kraków', 'Ihud Bnei Sakhnin', 'Maccabi Haifa'],
  },
  {
    id: 223,
    name: 'Marek Zienczuk',
    country: 'Polska',
    clubs: ['Ruch Chorzów', 'Wisła Kraków', 'Amica Wronki', 'AO Xanthi', 'Lechia Gdańsk'],
  },
  {
    id: 224,
    name: 'Deleu',
    country: 'Brazylia',
    clubs: [
      'Lechia Gdańsk',
      'Cracovia',
      'Bytovia Bytów',
      'Clube Náutico Capibaribe',
      'Chojniczanka Chojnice',
      'Miedź Legnica',
    ],
  },
  {
    id: 225,
    name: 'Ricardinho',
    country: 'Brazylia',
    clubs: [
      'FC Sheriff Tyraspol',
      'Wisła Płock',
      'ŁKS Łódź',
      'Lechia Gdańsk',
      'Khor Fakkan',
      'Górnik Łęczna',
      'Sharjah FC',
      'FC Tosno',
      'Crvena Zvezda Belgrad',
      'EC Santo André',
    ],
  },
  {
    id: 226,
    name: 'Damian Byrtek',
    country: 'Polska',
    clubs: [
      'Wisła Płock',
      'Chojniczanka Chojnice',
      'Chrobry Głogów',
      'Raków Częstochowa',
      'Stomil Olsztyn',
      'Podbeskidzie Bielsko-Biała',
      'Okocimski KS Brzesko',
      'Miedź Legnica',
      'Piast Gliwice',
    ],
  },
  {
    id: 227,
    name: 'José Kanté',
    country: 'Gwinea',
    clubs: [
      'Wisła Płock',
      'Legia Warszawa',
      'Kairat Almaty',
      'AE Prat',
      'AEK Larnaka',
      'Górnik Zabrze',
      'Cangzhou Mighty Lions',
      'Gimnàstic de Tarragona',
      'Urawa Red Diamonds',
    ],
  },
  {
    id: 228,
    name: 'Giorgi Merebashvili',
    country: 'Gruzja',
    clubs: [
      'Dinamo Tbilisi',
      'Wisła Płock',
      'Podbeskidzie Bielsko-Biała',
      'Sandecja Nowy Sącz',
      'FK Vojvodina Novi Sad',
    ],
  },
  {
    id: 229,
    name: 'Thomas Dähne',
    country: 'Niemcy',
    clubs: ['HJK Helsinki', 'Wisła Płock', 'Holstein Kiel', 'Red Bull Salzburg'],
  },
  {
    id: 230,
    name: 'Przemysław Szyminski',
    country: 'Polska',
    clubs: ['Frosinone Calcio', 'Wisła Płock', 'Rozwój Katowice', 'Palermo FC'],
  },
  {
    id: 231,
    name: 'Mateusz Praszelik',
    country: 'Polska',
    clubs: ['Śląsk Wrocław', 'Cosenza Calcio', 'Legia Warszawa'],
  },
  {
    id: 232,
    name: 'Lukasz Broz',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'Legia Warszawa', 'Śląsk Wrocław', 'Mamry Giżycko', 'KS Kutno'],
  },
  {
    id: 233,
    name: 'Kevin Kampl',
    country: 'Słowenia',
    clubs: [
      'RasenBallsport Leipzig',
      'Red Bull Salzburg',
      'Bayer 04 Leverkusen',
      'VfL Osnabrück',
      'Borussia Dortmund',
      'VfR Aalen',
      'SpVgg Greuther Fürth',
    ],
  },
  {
    id: 234,
    name: 'Ebi Smolarek',
    country: 'Polska',
    clubs: [
      'Jagiellonia Białystok',
      'Borussia Dortmund',
      'Feyenoord Rotterdam',
      'Racing Santander',
      'Polonia Warszawa',
      'ADO Den Haag',
      'Bolton Wanderers',
    ],
  },
  {
    id: 235,
    name: 'Grzegorz Rasiak',
    country: 'Polska',
    clubs: [
      'FC Southampton',
      'Dyskobolia Grodzisk Wielkopolski',
      'Derby County',
      'FC Reading',
      'Odra Wodzisław Śląski',
      'Warta Poznań',
      'FC Watford',
      'AEL Limassol',
      'Lechia Gdańsk',
      'Jagiellonia Białystok',
      'Tottenham Hotspur',
      'Bolton Wanderers',
      'GKS Bełchatów',
    ],
  },
  {
    id: 236,
    name: 'Maciej Zurawski',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Celtic Glasgow', 'Lech Poznań', 'AE Larisa', 'Omonia Nikozja'],
  },
  {
    id: 237,
    name: 'Paweł Brożek',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Trabzonspor', 'GKS Katowice', 'Recreativo Huelva', 'Celtic Glasgow'],
  },
  {
    id: 238,
    name: 'Patryk Klimala',
    country: 'Polska',
    clubs: ['New York Red Bulls', 'Jagiellonia Białystok', 'Celtic Glasgow', 'Wigry Suwałki', 'Hapoel Beer Sheva'],
  },
  {
    id: 239,
    name: 'Łukasz Załuska.',
    country: 'Polska',
    clubs: [
      'Pogoń Szczecin',
      'Dundee United FC',
      'Celtic Glasgow',
      'Miedź Legnica',
      'Wisła Kraków',
      'Korona Kielce',
      'Stomil Olsztyn',
      'SV Darmstadt 98',
    ],
  },
  {
    id: 240,
    name: 'Pawel Jaroszynski',
    country: 'Polska',
    clubs: ['Cracovia', 'US Salernitana 1919', 'Chievo Verona', 'Delfino Pescara 1936'],
  },
  {
    id: 241,
    name: 'Filip Jagiello',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'Brescia Calcio', 'Genoa CFC'],
  },
  {
    id: 242,
    name: 'Adam Dźwigała',
    country: 'Polska',
    clubs: [
      'FC St. Pauli',
      'Wisła Płock',
      'Jagiellonia Białystok',
      'Desportivo Aves',
      'Górnik Łęczna',
      'Lechia Gdańsk',
      'Górnik Zabrze',
    ],
  },
  {
    id: 243,
    name: 'Jakub Piotrowski',
    country: 'Polska',
    clubs: ['Ludogorets Razgrad', 'Fortuna Düsseldorf', 'Pogoń Szczecin', 'KRC Genk', 'Stomil Olsztyn', 'SK Beveren'],
  },
  {
    id: 244,
    name: 'Ireneusz Jeleń',
    country: 'Polska',
    clubs: ['AJ Auxerre', 'Wisła Płock', 'LOSC Lille', 'Górnik Zabrze', 'Podbeskidzie Bielsko-Biała'],
  },
  {
    id: 245,
    name: 'Dawid Janczyk',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'KSC Lokeren',
      'CSKA Moskwa',
      'Beerschot AC',
      'Sandecja Nowy Sącz',
      'Korona Kielce',
      'Piast Gliwice',
    ],
  },
  {
    id: 246,
    name: 'Jerzy Dudek',
    country: 'Polska',
    clubs: ['FC Liverpool', 'Feyenoord Rotterdam', 'Real Madryt', 'GKS Tychy'],
  },
  {
    id: 247,
    name: 'Michal Janota',
    country: 'Polska',
    clubs: [
      'Korona Kielce',
      'Podbeskidzie Bielsko-Biała',
      'Go Ahead Eagles Deventer',
      'Arka Gdynia',
      'Excelsior Rotterdam',
      'Stal Mielec',
      'Central Coast Mariners',
      'Górnik Zabrze',
      'Feyenoord Rotterdam',
    ],
  },
  {
    id: 248,
    name: 'Paweł Abott',
    country: 'Polska',
    clubs: [
      'Huddersfield Town',
      'Arka Gdynia',
      'FC Darlington',
      'Ruch Chorzów',
      'Oldham Athletic',
      'Zawisza Bydgoszcz',
      'Preston North End',
      'Charlton Athletic',
      'Swansea City',
      'Stomil Olsztyn',
      'ŁKS Łódź',
    ],
  },
  {
    id: 249,
    name: 'Marcin Adamski',
    country: 'Polska',
    clubs: ['SK Rapid Wiedeń', 'Zagłębie Lubin', 'ŁKS Łódź', 'FC Erzgebirge Aue', 'Angers SCO'],
  },
  {
    id: 250,
    name: 'Tomasz Cywka',
    country: 'Polska',
    clubs: [
      'Wisła Kraków',
      'FC Barnsley',
      'Chrobry Głogów',
      'Derby County',
      'Lech Poznań',
      'FC Blackpool',
      'AFC Rochdale',
      'FC Reading',
      'Oldham Athletic',
      'Wigan Athletic',
    ],
  },
  {
    id: 251,
    name: 'Jarosław Fojut',
    country: 'Polska',
    clubs: [
      'Pogoń Szczecin',
      'Śląsk Wrocław',
      'Dundee United FC',
      'Tromsø IL',
      'Luton Town',
      'Stal Rzeszów',
      'Wisła Płock',
      'Bolton Wanderers',
      'Stockport County',
    ],
  },
  {
    id: 252,
    name: 'Marek Saganowski',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'ŁKS Łódź',
      'FC Southampton',
      'Atromitos Ateny',
      'Vitoria Guimarães SC',
      'Odra Wodzisław Śląski',
      'Aalborg BK',
      'Wisła Płock',
      'Feyenoord Rotterdam',
      'ESTAC Troyes',
      'Hamburger SV',
    ],
  },
  {
    id: 253,
    name: 'Artur Wichniarek',
    country: 'Polska',
    clubs: ['Arminia Bielefeld', 'Hertha BSC', 'Widzew Łódź', 'Lech Poznań'],
  },
  {
    id: 254,
    name: 'Piotr Brozek',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Piast Gliwice', 'Lechia Gdańsk', 'Górnik Zabrze', 'Trabzonspor'],
  },
  {
    id: 255,
    name: 'Marcin Zewlakow',
    country: 'Polska',
    clubs: [
      'Excelsior Mouscron',
      'APOEL Nikosia',
      'Polonia Warszawa',
      'GKS Bełchatów',
      'KAA Gent',
      'KSK Beveren',
      'FC Metz',
      'Korona Kielce',
    ],
  },
  {
    id: 256,
    name: 'Michal Zewlakow',
    country: 'Polska',
    clubs: [
      'Olympiakos Pireus',
      'RSC Anderlecht',
      'Excelsior Mouscron',
      'Polonia Warszawa',
      'Legia Warszawa',
      'KSK Beveren',
      'MKE Ankaragücü',
    ],
  },
  {
    id: 257,
    name: 'Michal Mak',
    country: 'Polska',
    clubs: [
      'GKS Bełchatów',
      'Lechia Gdańsk',
      'Górnik Łęczna',
      'Ruch Radzionków',
      'Wisła Kraków',
      'Wieczysta Kraków',
      'Śląsk Wrocław',
      'Arminia Bielefeld',
    ],
  },
  {
    id: 258,
    name: 'Bartosz Slusarski',
    country: 'Polska',
    clubs: [
      'Lech Poznań',
      'Dyskobolia Grodzisk Wielkopolski',
      'Cracovia',
      'GKS Bełchatów',
      'UD Leiria',
      'Miedź Legnica',
      'Arka Gdynia',
      'Sheffield Wednesday',
      'Widzew Łódź',
      'FC Blackpool',
      'West Bromwich Albion',
    ],
  },
  {
    id: 259,
    name: 'Jacek Bak',
    country: 'Polska',
    clubs: ['Olympique Lyon', 'FK Austria Wiedeń', 'RC Lens', 'Lech Poznań'],
  },
  {
    id: 260,
    name: 'Tomasz Rząsa',
    country: 'Polska',
    clubs: [
      'Feyenoord Rotterdam',
      'SV Ried',
      'De Graafschap Doetinchem',
      'Grasshopper Club Zurych',
      'SC Heerenveen',
      'ADO Den Haag',
      'BSC Young Boys',
      'FC Lugano',
      'FK Partizan Belgrad',
      'Sokol Pniewy',
    ],
  },
  {
    id: 261,
    name: 'Marcin Wasilewski',
    country: 'Polska',
    clubs: [
      'RSC Anderlecht',
      'Leicester City',
      'Wisła Płock',
      'Wisła Kraków',
      'Śląsk Wrocław',
      'Amica Wronki',
      'Lech Poznań',
    ],
  },
  {
    id: 262,
    name: 'Emmanuel Olisadebe',
    country: 'Polska',
    clubs: [
      'Panathinaikos Ateny',
      'Polonia Warszawa',
      'Henan FC',
      'APOP Kinyras Pegias',
      'AO Xanthi',
      'Veria NPS',
      'FC Portsmouth',
    ],
  },
  {
    id: 263,
    name: 'Ludovic Obraniak',
    country: 'Polska',
    clubs: [
      'LOSC Lille',
      'FC Metz',
      'FC Girondins Bordeaux',
      'AJ Auxerre',
      'Maccabi Haifa',
      'Caykur Rizespor',
      'SV Werder Bremen',
    ],
  },
  {
    id: 264,
    name: 'Krzysztof Krol',
    country: 'Polska',
    clubs: [
      'Podbeskidzie Bielsko-Biała',
      'Jagiellonia Białystok',
      'Piast Gliwice',
      'Chicago Fire FC',
      'CF Montréal',
      'AEL Kalloni',
      'FC Sheriff Tyraspol',
      'Polonia Bytom',
      'Montreal Impact Reserves',
      'Dyskobolia Grodzisk Wielkopolski',
    ],
  },
  {
    id: 265,
    name: 'Radoslaw Matusiak',
    country: 'Polska',
    clubs: [
      'GKS Bełchatów',
      'Cracovia',
      'Widzew Łódź',
      'Wisła Płock',
      'SC Heerenveen',
      'Szczakowianka Jaworzno',
      'Wisła Kraków',
      'ŁKS Łódź',
      'Asteras Tripolis',
      'Palermo FC',
    ],
  },
  {
    id: 266,
    name: 'Timo Werner',
    country: 'Niemcy',
    clubs: ['RasenBallsport Leipzig', 'VfB Stuttgart', 'Chelsea FC'],
  },
  {
    id: 267,
    name: 'André Silva',
    country: 'Portugalia',
    clubs: ['RasenBallsport Leipzig', 'Eintracht Frankfurt', 'FC Porto', 'AC Milan', 'Sevilla FC'],
  },
  {
    id: 268,
    name: 'Maciej Jankowski',
    country: 'Polska',
    clubs: ['Ruch Chorzów', 'Arka Gdynia', 'Piast Gliwice', 'Wisła Kraków', 'Stal Mielec', 'Wieczysta Kraków'],
  },
  {
    id: 269,
    name: 'Karol Fila',
    country: 'Polska',
    clubs: ['Lechia Gdańsk', 'RC Strasbourg Alsace', 'Chojniczanka Chojnice', 'SV Zulte Waregem'],
  },
  {
    id: 270,
    name: 'Rafal Straczek',
    country: 'Polska',
    clubs: ['Stal Mielec', 'JKS 1909 Jarosław', 'Wólczanka Wólka Pełkińska', 'FC Girondins Bordeaux', 'Motor Lublin'],
  },
  {
    id: 271,
    name: 'Lukasz Poreba',
    country: 'Polska',
    clubs: ['Zagłębie Lubin', 'RC Lens'],
  },
  {
    id: 272,
    name: 'Pawel Bochniewicz',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'SC Heerenveen', 'Recreativo Granada', 'Reggina 1914', 'Udinese Calcio'],
  },
  {
    id: 273,
    name: 'Neymar',
    country: 'Brazylia',
    clubs: ['FC Barcelona', 'FC Paris Saint-Germain', 'Santos FC'],
  },
  {
    id: 274,
    name: 'Kylian Mbappé',
    country: 'Francja',
    clubs: ['FC Paris Saint-Germain', 'AS Monaco'],
  },
  {
    id: 275,
    name: 'Achraf Hakimi',
    country: 'Maroko',
    clubs: ['FC Paris Saint-Germain', 'Borussia Dortmund', 'Inter Mediolan', 'Real Madryt'],
  },
  {
    id: 276,
    name: 'Edinson Cavani',
    country: 'Urugwaj',
    clubs: ['FC Paris Saint-Germain', 'SSC Napoli', 'Palermo FC', 'Manchester United', 'Valencia CF'],
  },
  {
    id: 277,
    name: 'Ángel Di María',
    country: 'Argentyna',
    clubs: [
      'FC Paris Saint-Germain',
      'Real Madryt',
      'Benfika Lizbona',
      'Juventus Turyn',
      'CA Rosario Central',
      'Manchester United',
    ],
  },
  {
    id: 278,
    name: 'Mauro Icardi',
    country: 'Argentyna',
    clubs: ['Inter Mediolan', 'FC Paris Saint-Germain', 'UC Sampdoria', 'Galatasaray'],
  },
  {
    id: 279,
    name: 'David Luiz',
    country: 'Brazylia',
    clubs: ['Chelsea FC', 'Benfika Lizbona', 'FC Paris Saint-Germain', 'Flamengo Rio de Janeiro', 'FC Arsenal'],
  },
  {
    id: 280,
    name: 'Thiago Silva',
    country: 'Brazylia',
    clubs: ['FC Paris Saint-Germain', 'AC Milan', 'Chelsea FC', 'Fluminense Football Club'],
  },
  {
    id: 281,
    name: 'Javier Pastore',
    country: 'Argentyna',
    clubs: ['FC Paris Saint-Germain', 'Palermo FC', 'AS Roma', 'CA Huracán', 'Elche CF', 'Qatar SC'],
  },
  {
    id: 282,
    name: 'Nicolas Anelka',
    country: 'Francja',
    clubs: [
      'Chelsea FC',
      'Manchester City',
      'FC Arsenal',
      'FC Paris Saint-Germain',
      'Bolton Wanderers',
      'Fenerbahce',
      'Real Madryt',
      'Shanghai Shenhua',
      'FC Liverpool',
      'West Bromwich Albion',
      'Juventus Turyn',
    ],
  },
  {
    id: 283,
    name: 'Renato Sanches',
    country: 'Portugalia',
    clubs: ['LOSC Lille', 'Bayern Monachium', 'Benfika Lizbona', 'FC Paris Saint-Germain', 'Swansea City'],
  },
  {
    id: 284,
    name: 'Fabián Ruiz',
    country: 'Hiszpania',
    clubs: ['SSC Napoli', 'Real Betis Balompié', 'FC Paris Saint-Germain', 'Betis Deportivo Balompié', 'Elche CF'],
  },
  {
    id: 285,
    name: 'Malang Sarr',
    country: 'Francja',
    clubs: ['OGC Nice', 'Chelsea FC', 'FC Porto', 'AS Monaco'],
  },
  {
    id: 286,
    name: 'Ruben Aguilar',
    country: 'Francja',
    clubs: ['AS Monaco', 'AJ Auxerre', 'Montpellier HSC', 'AS Saint-Étienne'],
  },
  {
    id: 287,
    name: 'Youssouf Fofana',
    country: 'Francja',
    clubs: ['AS Monaco', 'RC Strasbourg Alsace'],
  },
  {
    id: 288,
    name: 'Wissam Ben Yedder',
    country: 'Francja',
    clubs: ['FC Toulouse', 'AS Monaco', 'Sevilla FC'],
  },
  {
    id: 289,
    name: 'Gelson Martins',
    country: 'Portugalia',
    clubs: ['Sporting CP', 'AS Monaco', 'Atlético Madryt'],
  },
  {
    id: 290,
    name: 'Edouard Mendy',
    country: 'Senegal',
    clubs: ['Chelsea FC', 'Stade Reims', 'Stade Rennais FC', 'AS Cherbourg'],
  },
  {
    id: 291,
    name: 'Eric Bailly',
    country: 'Wybrzeże Kości Słoniowej',
    clubs: ['Manchester United', 'Villarreal CF', 'Olympique Marseille', 'RCD Espanyol'],
  },
  {
    id: 292,
    name: 'Aurélien Tchouaméni',
    country: 'Francja',
    clubs: ['AS Monaco', 'Real Madryt', 'FC Girondins Bordeaux'],
  },
  {
    id: 293,
    name: 'Antonio Rüdiger',
    country: 'Niemcy',
    clubs: ['Chelsea FC', 'VfB Stuttgart', 'AS Roma', 'Real Madryt'],
  },
  {
    id: 294,
    name: 'Reinier',
    country: 'Brazylia',
    clubs: ['Borussia Dortmund', 'Girona FC', 'Flamengo Rio de Janeiro', 'Real Madryt'],
  },
  {
    id: 295,
    name: 'Andreas Christensen',
    country: 'Dania',
    clubs: ['Chelsea FC', 'Borussia Mönchengladbach', 'FC Barcelona'],
  },
  {
    id: 296,
    name: 'Philippe Coutinho',
    country: 'Brazylia',
    clubs: [
      'FC Liverpool',
      'Inter Mediolan',
      'FC Barcelona',
      'Aston Villa',
      'Bayern Monachium',
      'Clube de Regatas Vasco da Gama',
      'RCD Espanyol',
    ],
  },
  {
    id: 297,
    name: 'Marcos Alonso',
    country: 'Hiszpania',
    clubs: ['Chelsea FC', 'AC Fiorentina', 'Bolton Wanderers', 'FC Barcelona', 'AFC Sunderland', 'Real Madryt'],
  },
  {
    id: 298,
    name: 'Jules Koundé',
    country: 'Francja',
    clubs: ['Sevilla FC', 'FC Girondins Bordeaux', 'FC Barcelona'],
  },
  {
    id: 299,
    name: 'Miralem Pjanic',
    country: 'Bośnia i Hercegowina',
    clubs: ['AS Roma', 'Juventus Turyn', 'Olympique Lyon', 'FC Metz', 'FC Barcelona', 'Besiktas JK'],
  },
  {
    id: 300,
    name: 'Dimitri Payet',
    country: 'Fancja',
    clubs: ['Olympique Marseille', 'AS Saint-Étienne', 'LOSC Lille', 'FC Nantes'],
  },
  {
    id: 301,
    name: 'Florian Thauvin',
    country: 'Francja',
    clubs: [
      'Olympique Marseille',
      'SC Bastia',
      'Tigres UANL',
      'Udinese Calcio',
      'Newcastle United',
      'Grenoble Foot 38',
    ],
  },
  {
    id: 302,
    name: 'Michy Batshuayi',
    country: 'Belgia',
    clubs: [
      'Standard Liège',
      'Olympique Marseille',
      'Chelsea FC',
      'Besiktas JK',
      'Crystal Palace',
      'Fenerbahce',
      'Valencia CF',
      'Borussia Dortmund',
    ],
  },
  {
    id: 303,
    name: 'César Azpilicueta',
    country: 'Hispzania',
    clubs: ['Chelsea FC', 'CA Osasuna', 'Olympique Marseille'],
  },
  {
    id: 304,
    name: 'Kasper Schmeichel',
    country: 'Dania',
    clubs: ['Leicester City', 'Notts County', 'OGC Nice', 'Leeds United', 'FC Bury', 'Cardiff City', 'Manchester City'],
  },
  {
    id: 305,
    name: 'Marcin Bulka',
    country: 'Polska',
    clubs: ['OGC Nice', 'LB Châteauroux', 'FC Cartagena', 'FC Paris Saint-Germain'],
  },
  {
    id: 306,
    name: 'Nicolas Pépé',
    country: 'Wybrzeże kości słoniowej',
    clubs: ['FC Arsenal', 'LOSC Lille', 'Angers SCO', 'US Orléans', 'OGC Nice'],
  },
  {
    id: 307,
    name: 'Aaron Ramsey',
    country: 'Walia',
    clubs: ['FC Arsenal', 'Juventus Turyn', 'OGC Nice', 'Cardiff City', 'Glasgow Rangers', 'Nottingham Forest'],
  },
  {
    id: 308,
    name: 'Alexandre Lacazette',
    country: 'Francja',
    clubs: ['Olympique Lyon', 'FC Arsenal'],
  },
  {
    id: 309,
    name: 'Corentin Tolisso',
    country: 'Francja',
    clubs: ['Olympique Lyon', 'Bayern Monachium'],
  },
  {
    id: 310,
    name: 'Nicolás Tagliafico',
    country: 'Argentyna',
    clubs: ['Ajax Amsterdam', 'CA Independiente', 'Olympique Lyon', 'Real Murcia CF'],
  },
  {
    id: 311,
    name: 'Jérôme Boateng',
    country: 'Niemcy',
    clubs: ['Bayern Monachium', 'Hamburger SV', 'Olympique Lyon', 'Manchester City', 'Hertha BSC'],
  },
  {
    id: 312,
    name: 'Moussa Dembélé',
    country: 'Francja',
    clubs: ['Olympique Lyon', 'Celtic Glasgow', 'FC Fulham', 'Atlético Madryt'],
  },
  {
    id: 313,
    name: 'Piotr Swierczewski',
    country: 'Polska',
    clubs: [
      'SC Bastia',
      'Lech Poznań',
      'AS Saint-Étienne',
      'Dyskobolia Grodzisk Wielkopolski',
      'Olympique Marseille',
      'ŁKS Łódź',
      'Zagłębie Lubin',
      'GKS Katowice',
      'Korona Kielce',
      'Polonia Warszawa',
      'Birmingham City',
    ],
  },
  {
    id: 314,
    name: 'Dariusz Dudka',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'AJ Auxerre', 'Amica Wronki', 'Lech Poznań', 'Levante UD', 'Birmingham City'],
  },
  {
    id: 315,
    name: 'Tomasz Klos',
    country: 'Polska',
    clubs: ['ŁKS Łódź', 'Wisła Kraków', 'AJ Auxerre', '1.FC Kaiserslautern', '1.FC Köln'],
  },
  {
    id: 316,
    name: 'Roman Kosecki',
    country: 'Polska',
    clubs: [
      'Atlético Madryt',
      'Chicago Fire FC',
      'Galatasaray',
      'FC Nantes',
      'Montpellier HSC',
      'CA Osasuna',
      'Legia Warszawa',
    ],
  },
  {
    id: 317,
    name: 'Tomasz Frankowski',
    country: 'Polska',
    clubs: [
      'Wisła Kraków',
      'Jagiellonia Białystok',
      'RC Strasbourg Alsace',
      'FC Martigues',
      'CD Tenerife',
      'Wolverhampton Wanderers',
      'Chicago Fire FC',
      'Nagoya Grampus',
      'Elche CF',
    ],
  },
  {
    id: 318,
    name: 'Mariusz Stepinski',
    country: 'Polska',
    clubs: [
      'Chievo Verona',
      'Aris Limassol',
      'Ruch Chorzów',
      'Widzew Łódź',
      'US Lecce',
      'FC Nantes',
      'Wisła Kraków',
      'Hellas Verona',
    ],
  },
  {
    id: 319,
    name: 'Marcin Kuzba',
    country: 'Polska',
    clubs: [
      'FC Lausanne-Sport',
      'Górnik Zabrze',
      'Wisła Kraków',
      'AS Saint-Étienne',
      'Olympiakos Pireus',
      'AJ Auxerre',
    ],
  },
  {
    id: 320,
    name: 'Andrzej Szarmach',
    country: 'Polska',
    clubs: ['AJ Auxerre', 'Górnik Zabrze', 'Stal Mielec', 'EA Guingamp'],
  },
  {
    id: 321,
    name: 'Ismaily',
    country: 'Brazylia',
    clubs: ['Szachtar Donieck', 'SC Ollanęs', 'LOSC Lille', 'SC Braga'],
  },
  {
    id: 322,
    name: 'Adam Ounas',
    country: 'Algieria',
    clubs: ['SSC Napoli', 'FC Girondins Bordeaux', 'LOSC Lille', 'OGC Nice', 'FC Crotone', 'Cagliari Calcio'],
  },
  {
    id: 323,
    name: 'Adam Buksa',
    country: 'Polska',
    clubs: [
      'New England Revolution',
      'Pogoń Szczecin',
      'Zagłębie Lubin',
      'Lechia Gdańsk',
      'Novara Primavera',
      'RC Lens',
    ],
  },
  {
    id: 324,
    name: 'Jean Onana',
    country: 'Kamerun',
    clubs: ['Royal Excel Mouscron', 'FC Girondins Bordeaux', 'RC Lens', 'LOSC Lille'],
  },
  {
    id: 325,
    name: 'Brice Samba',
    country: 'Francja',
    clubs: ['SM Caen', 'Nottingham Forest', 'RC Lens', 'AS Nancy-Lorraine', 'Olympique Marseille', 'AC Le Havre'],
  },
  {
    id: 326,
    name: 'Mateusz Lis',
    country: 'Polska',
    clubs: [
      'Wisła Kraków',
      'Altay SK',
      'Raków Częstochowa',
      'ESTAC Troyes',
      'Miedź Legnica',
      'Podbeskidzie Bielsko-Biała',
    ],
  },
  {
    id: 327,
    name: 'Michal Karbownik',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Fortuna Düsseldorf', 'Olympiakos Pireus', 'Brighton & Hove Albion'],
  },
  {
    id: 328,
    name: 'Sebastian Szymanski',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'Dynamo Moskwa', 'Feyenoord Rotterdam'],
  },
  {
    id: 329,
    name: 'Ondrej Duda',
    country: 'Polska',
    clubs: ['Legia Warszawa', '1.FC Köln', 'Hertha BSC', 'FC VSS Kosice', 'Hellas Verona', 'Norwich City'],
  },
  {
    id: 330,
    name: 'Jaroslaw Niezgoda',
    country: 'Polska',
    clubs: ['Portland Timbers', 'Wisła Puławy', 'Legia Warszawa', 'Ruch Chorzów'],
  },
  {
    id: 331,
    name: 'Wojciech Kowalczyk',
    country: 'Polska',
    clubs: ['Real Betis Balompié', 'Legia Warszawa', 'UD Las Palmas', 'Anorthosis Famagusta', 'APOEL Nikosia'],
  },
  {
    id: 332,
    name: 'Marcin Komorowski',
    country: 'Polska',
    clubs: ['Akhmat Grozny', 'Legia Warszawa', 'Polonia Bytom', 'ŁKS Łódź', 'Zagłębie Sosnowiec', 'GKS Bełchatów'],
  },
  {
    id: 333,
    name: 'Bartosz Karwan',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'GKS Katowice', 'Arka Gdynia', 'Hertha BSC'],
  },
  {
    id: 334,
    name: 'Roger Guerreiro',
    country: 'Polska',
    clubs: ['Legia Warszawa', 'AEK Ateny', 'Sport Club Corinthians Paulista', 'Aris Limassol', 'Celta de Vigo'],
  },
  {
    id: 335,
    name: 'Maciej Iwanski',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'Legia Warszawa',
      'Manisaspor',
      'Podbeskidzie Bielsko-Biała',
      'Szczakowianka Jaworzno',
      'Ruch Chorzów',
      'ŁKS Łódź',
    ],
  },
  {
    id: 336,
    name: 'Jakub Kosecki',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Adana Demirspor',
      'SV Sandhausen',
      'Śląsk Wrocław',
      'ŁKS Łódź',
      'Motor Lublin',
      'Lechia Gdańsk',
      'Cracovia',
    ],
  },
  {
    id: 337,
    name: 'Maciej Korzym',
    country: 'Polska',
    clubs: [
      'Korona Kielce',
      'Sandecja Nowy Sącz',
      'Odra Wodzisław Śląski',
      'Legia Warszawa',
      'Podbeskidzie Bielsko-Biała',
      'GKS Bełchatów',
      'Górnik Zabrze',
    ],
  },
  {
    id: 338,
    name: 'Adam Fraczczak',
    country: 'Polska',
    clubs: ['Pogoń Szczecin', 'Korona Kielce', 'Ząbkovia Ząbki', 'Kotwica Kołobrzeg'],
  },
  {
    id: 339,
    name: 'Piotr Reiss',
    country: 'Polska',
    clubs: ['Lech Poznań', 'Warta Poznań', 'MSV Duisburg', 'Hertha BSC', 'SpVgg Greuther Fürth'],
  },
  {
    id: 340,
    name: 'Bartosz Bosacki',
    country: 'Polska',
    clubs: ['Lech Poznań', 'Amica Wronki', '1.FC Nürnberg'],
  },
  {
    id: 341,
    name: 'Miroslaw Szymkowiak',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'Wisła Kraków', 'Trabzonspor'],
  },
  {
    id: 342,
    name: 'Patryk Malecki',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Zagłębie Sosnowiec', 'Stal Rzeszów', 'Pogoń Szczecin', 'Spartak Trnava', 'Eskisehirspor'],
  },
  {
    id: 343,
    name: 'Kamil Kosowski',
    country: 'Polska',
    clubs: [
      'Wisła Kraków',
      'Górnik Zabrze',
      '1.FC Kaiserslautern',
      'APOEL Nikosia',
      'GKS Bełchatów',
      'Chievo Verona',
      'Apollon Limassol',
      'FC Southampton',
      'Cádiz CF',
    ],
  },
  {
    id: 344,
    name: 'Adam Kokoszka',
    country: 'Polska',
    clubs: [
      'Śląsk Wrocław',
      'FC Empoli',
      'Polonia Warszawa',
      'Wisła Kraków',
      'Torpedo Moskau',
      'Unia Tarnów',
      'Zagłębie Sosnowiec',
    ],
  },
  {
    id: 345,
    name: 'Lukas Klemenz',
    country: 'Polska',
    clubs: [
      'Wisła Kraków',
      'GKS Katowice',
      'GKS Bełchatów',
      'Korona Kielce',
      'Olimpia Grudziądz',
      'Jagiellonia Białystok',
    ],
  },
  {
    id: 346,
    name: 'Andrzej Niedzielan',
    country: 'Polska',
    clubs: [
      'NEC Nijmegen',
      'Ruch Chorzów',
      'Wisła Kraków',
      'Korona Kielce',
      'Dyskobolia Grodzisk Wielkopolski',
      'Górnik Zabrze',
      'Zagłębie Lubin',
      'Cracovia',
    ],
  },
  {
    id: 347,
    name: 'Adrian Mierzejewski',
    country: 'Polska',
    clubs: [
      'Trabzonspor',
      'Wisła Płock',
      'Polonia Warszawa',
      'Al-Nassr FC',
      'Chongqing Liangjiang Athletic',
      'Sydney FC',
    ],
  },
  {
    id: 348,
    name: 'Zbigniew Boniek',
    country: 'Polska',
    clubs: ['Juventus Turyn', 'AS Roma', 'Widzew Łódź'],
  },
  {
    id: 349,
    name: 'Wojciech Golla',
    country: 'Polska',
    clubs: ['NEC Nijmegen', 'Śląsk Wrocław', 'Pogoń Szczecin'],
  },
  {
    id: 350,
    name: 'Filip Bednarek',
    country: 'Polska',
    clubs: ['Lech Poznań', 'De Graafschap Doetinchem', 'SC Heerenveen', 'FC Twente Enschede'],
  },
  {
    id: 351,
    name: 'Arkadiusz Radomski',
    country: 'Polska',
    clubs: ['SC Heerenveen', 'FK Austria Wiedeń', 'NEC Nijmegen', 'Cracovia'],
  },
  {
    id: 352,
    name: 'Tomasz Iwan',
    country: 'Polska',
    clubs: ['PSV Eindhoven', 'Feyenoord Rotterdam', 'Lech Poznań', 'FK Austria Wiedeń'],
  },
  {
    id: 353,
    name: 'Włodzimierz Smolarek',
    country: 'Polska',
    clubs: ['FC Utrecht', 'Eintracht Frankfurt', 'Feyenoord Rotterdam', 'Widzew Łódź'],
  },
  {
    id: 354,
    name: 'Pawel Kieszek',
    country: 'Polska',
    clubs: [
      'Córdoba CF',
      'Rió Ave FC',
      'Vitoria Setúbal FC',
      'Polonia Warszawa',
      'SC Braga',
      'Wisła Kraków',
      'Málaga CF',
      'FC Porto',
    ],
  },
  {
    id: 355,
    name: 'Jan Urban',
    country: 'Polska',
    clubs: ['CA Osasuna', 'Górnik Zabrze', 'VfB Oldenburg', 'Real Valladolid CF'],
  },
  {
    id: 356,
    name: 'Cezary Wilk',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'Korona Kielce', 'Deportivo de La Coruña', 'ŁKS Łódź', 'Real Zaragoza'],
  },
  {
    id: 357,
    name: 'Lumir Sedlacek',
    country: 'Czechy',
    clubs: [
      'Slezsky FC Opava',
      'SK Slavia Praga',
      'Wisła Płock',
      'Dyskobolia Grodzisk Wielkopolski',
      'Polonia Warszawa',
      'SK Dynamo Czeskie Budziejowice',
      'Piast Gliwice',
      'FC Hradec Kralove',
    ],
  },
  {
    id: 358,
    name: 'Alberto Cifuentes',
    country: 'Hiszpania',
    clubs: ['Cádiz CF', 'Real Murcia CF', 'Piast Gliwice', 'Rayo Vallecano'],
  },
  {
    id: 359,
    name: 'Collins John',
    country: 'Holandia',
    clubs: [
      'FC Fulham',
      'FC Twente Enschede',
      'Chicago Fire FC',
      'Pittsburgh Riverhounds SC',
      'Leicester City',
      'NEC Nijmegen',
      'FC Watford',
      'FK Qabala',
      'Piast Gliwice',
    ],
  },
  {
    id: 360,
    name: 'Rubén Jurado',
    country: 'Hiszpania',
    clubs: ['Piast Gliwice', 'Atlético Baleares', 'AEL Limassol', 'Arka Gdynia'],
  },
  {
    id: 361,
    name: 'Mikkel Kirkeskov',
    country: 'Dania',
    clubs: ['Aarhus GF', 'Aalesunds FK', 'Piast Gliwice', 'Holstein Kiel', 'Odense Boldklub'],
  },
  {
    id: 362,
    name: 'Wilde-Donald Guerrier',
    country: 'Haiti',
    clubs: ['Qarabağ FK', 'Wisła Kraków', 'Alanyaspor', 'Olympiakos Nikosia', 'Neftchi Baku', 'Apollon Limassol'],
  },
  {
    id: 363,
    name: 'Kalu Uche',
    country: 'Nigeria',
    clubs: ['UD Almería', 'Wisła Kraków', 'FC Girondins Bordeaux', 'Kasimpasa', 'Levante UD', 'RCD Espanyol'],
  },
  {
    id: 364,
    name: 'Petar Brlek',
    country: 'Chorwacja',
    clubs: ['Slaven Belupo Koprivnica', 'Wisła Kraków', 'Genoa CFC'],
  },
  {
    id: 365,
    name: 'Marcelo',
    country: 'Brazylia',
    clubs: [
      'Olympique Lyon',
      'PSV Eindhoven',
      'Hannover 96',
      'Besiktas JK',
      'Wisła Kraków',
      'Santos FC',
      'FC Girondins Bordeaux',
    ],
  },
  {
    id: 366,
    name: 'Pedro Tiba',
    country: 'Portugalia',
    clubs: ['Lech Poznań', 'SC Braga', 'GD Chaves', 'Gil Vicente FC', 'Real Valladolid CF'],
  },
  {
    id: 367,
    name: 'Gergő Lovrencsics',
    country: 'Wegry',
    clubs: ['Ferencvárosi TC', 'Lech Poznań', 'HNK Hajduk Split'],
  },
  {
    id: 368,
    name: 'Paulus Arajuuri',
    country: 'Finlandia',
    clubs: ['Bröndby IF', 'Lech Poznań', 'Kalmar FF', 'HJK Helsinki'],
  },
  {
    id: 369,
    name: 'Barry Douglas',
    country: 'Szkocja',
    clubs: [
      'Lech Poznań',
      'Dundee United FC',
      'Konyaspor',
      'Leeds United',
      'Wolverhampton Wanderers',
      'Blackburn Rovers',
      "Queen's Park FC",
    ],
  },
  {
    id: 370,
    name: 'Joel Tshibamba',
    country: 'Demokratyczna Republika Kongo',
    clubs: ['AE Larisa', 'Lech Poznań', 'NEC Nijmegen', 'Arka Gdynia', 'FC Koper'],
  },
  {
    id: 371,
    name: 'Darko Jevtic',
    country: 'Szwajcaria',
    clubs: ['Lech Poznań', 'Rubin Kazań', 'AEK Ateny'],
  },
  {
    id: 372,
    name: 'Sergey Krivets',
    country: 'Białoruś',
    clubs: ['BATE Borysów', 'Lech Poznań', 'Dynamo Brest', 'FC Metz', 'Wisła Płock', 'Arka Gdynia'],
  },
  {
    id: 373,
    name: 'Abdul-Aziz Tetteh',
    country: 'Ghana',
    clubs: ['Lech Poznań', 'AO Platanias', 'Dynamo Moskwa', 'CD Leganés', 'Widzew Łódź', 'Gaziantep FK', 'Granada CF'],
  },
  {
    id: 374,
    name: 'Tamás Kádár',
    country: 'Wegry',
    clubs: ['Dynamo Kijów', 'Lech Poznań', 'Newcastle United', 'Huddersfield Town'],
  },
  {
    id: 375,
    name: 'Aleksandar Tonev',
    country: 'Bułgaria',
    clubs: ['Lech Poznań', 'CSKA-Sofia', 'FC Crotone', 'Frosinone Calcio', 'Aston Villa', 'Celtic Glasgow'],
  },
  {
    id: 376,
    name: 'Artjoms Rudnevs',
    country: 'Łotwa',
    clubs: ['Hamburger SV', 'Lech Poznań', '1.FC Köln', 'Hannover 96'],
  },
  {
    id: 377,
    name: 'Steeven Langil',
    country: 'Martynika',
    clubs: ['Ratchaburi FC', 'AJ Auxerre', 'SK Beveren', 'NEC Nijmegen', 'Legia Warszawa'],
  },
  {
    id: 378,
    name: 'Henrik Ojamaa',
    country: 'Estonia',
    clubs: ['FC Flora Tallinn', 'Motherwell FC', 'Legia Warszawa', 'Miedź Legnica', 'Widzew Łódź', 'HNK Gorica'],
  },
  {
    id: 379,
    name: 'André Martins',
    country: 'Portugalia',
    clubs: ['Legia Warszawa', 'Sporting CP', 'Olympiakos Pireus', 'Hapoel Beer Sheva', 'CF Os Belenenses'],
  },
  {
    id: 380,
    name: 'Aleksandar Vukovic',
    country: 'Serbia',
    clubs: ['Legia Warszawa', 'Korona Kielce', 'Iraklis Saloniki', 'GS Ergotelis', 'FK Partizan Belgrad'],
  },
  {
    id: 381,
    name: 'Danijel Ljuboja',
    country: 'Serbia',
    clubs: [
      'RC Strasbourg Alsace',
      'Legia Warszawa',
      'FC Sochaux-Montbéliard',
      'FC Paris Saint-Germain',
      'Grenoble Foot 38',
      'VfB Stuttgart',
      'OGC Nice',
      'RC Lens',
      'Hamburger SV',
      'VfL Wolfsburg',
    ],
  },
  {
    id: 382,
    name: 'Vladimer Dvalishvili',
    country: 'Gruzja',
    clubs: [
      'Maccabi Haifa',
      'Legia Warszawa',
      'Skonto Riga',
      'FK Atyrau',
      'Polonia Warszawa',
      'Pogoń Szczecin',
      'FC Saburtalo',
      'Dinamo Batumi',
      'Dinamo Tbilisi',
      'Odense Boldklub',
    ],
  },
  {
    id: 383,
    name: 'Adam Hlousek',
    country: 'Czechy',
    clubs: [
      'Legia Warszawa',
      'FK Jablonec',
      '1.FC Kaiserslautern',
      'SK Slavia Praga',
      '1.FC Nürnberg',
      'VfB Stuttgart',
      'FC Viktoria Pilzno',
      'Bruk-Bet Termalica Nieciecza',
    ],
  },
  {
    id: 384,
    name: 'Arvydas Novikovas',
    country: 'Litwa',
    clubs: ['Jagiellonia Białystok', 'Legia Warszawa', 'VfL Bochum', 'amsunspor'],
  },
  {
    id: 385,
    name: 'Dossa Júnior',
    country: 'Cypr',
    clubs: ['AEL Limassol', 'Legia Warszawa', 'AE Pafos', 'Digenis Akritas Morfou', 'Eskisehirspor', 'Konyaspor'],
  },
  {
    id: 386,
    name: 'Cafú',
    country: 'Portugalia',
    clubs: [
      'Vitoria Guimarães SC',
      'Legia Warszawa',
      'Nottingham Forest',
      'FC Lorient',
      'Olympiakos Pireus',
      'FC Metz',
    ],
  },
  {
    id: 387,
    name: 'Armando Sadiku',
    country: 'Albania',
    clubs: [
      'FC Lugano',
      'FC Zürich',
      'FC Locarno',
      'Málaga CF',
      'FC Cartagena',
      'UD Las Palmas',
      'Legia Warszawa',
      'FC Vaduz',
    ],
  },
  {
    id: 388,
    name: 'Thibault Moulin',
    country: 'Francja',
    clubs: [
      'Clermont Foot 63',
      'Legia Warszawa',
      'SM Caen',
      'SK Beveren',
      'Academica Clinceni',
      'LB Châteauroux',
      'MKE Ankaragücü',
      'Wieczysta Kraków',
      'PAOK Saloniki',
    ],
  },
  {
    id: 389,
    name: 'Orlando Sá',
    country: 'Portugalia',
    clubs: [
      'Standard Liège',
      'AEL Limassol',
      'Legia Warszawa',
      'Málaga CF',
      'FC Reading',
      'CD Nacional',
      'Maccabi Tel Aviv',
      'SC Braga',
      'FC Fulham',
      'FC Porto',
    ],
  },
  {
    id: 390,
    name: 'Vadis Odjidja-Ofoe',
    country: 'Belgia',
    clubs: [
      'FC Brügge',
      'KAA Gent',
      'Legia Warszawa',
      'Olympiakos Pireus',
      'Norwich City',
      'RSC Anderlecht',
      'Hamburger SV',
    ],
  },
  {
    id: 391,
    name: 'Aleksandar Prijovic',
    country: 'Serbia',
    clubs: [
      'PAOK Saloniki',
      'Legia Warszawa',
      'Al-Ittihad Club',
      'FC Sion',
      'Western United FC',
      'Tromsø IL',
      'Boluspor',
      'Djurgårdens IF',
    ],
  },
  {
    id: 392,
    name: 'Josip Juranovic',
    country: 'Chorwacja',
    clubs: ['HNK Hajduk Split', 'Celtic Glasgow', 'Legia Warszawa', '1.FC Union Berlin'],
  },
  {
    id: 393,
    name: 'Nemanja Nikolics',
    country: 'Węgry',
    clubs: [
      'MOL Fehérvár FC',
      'Chicago Fire FC',
      'Legia Warszawa',
      'Kaposvári Rákóczi FC',
      'AEK Larnaka',
      'Pendikspor',
    ],
  },
  {
    id: 394,
    name: 'Saïd Hamulic',
    country: 'Bosnia i Hercegowina',
    clubs: ['DFK Dainava Alytus', 'Stal Mielec', 'FK Suduva Marijampole', 'FC Toulouse'],
  },
  {
    id: 395,
    name: 'Vito Mannone',
    country: 'Włochy',
    clubs: [
      'AFC Sunderland',
      'FC Reading',
      'Minnesota United FC',
      'Hull City',
      'FC Arsenal',
      'FC Lorient',
      'Esbjerg fB',
      'AS Monaco',
      'FC Barnsley',
    ],
  },
  {
    id: 396,
    name: 'Julian Pollersbeck',
    country: 'Niemcy',
    clubs: ['Hamburger SV', '1.FC Kaiserslautern', 'Olympique Lyon'],
  },
  {
    id: 397,
    name: 'Yvon Mvogo',
    country: 'Szwajcaria',
    clubs: ['BSC Young Boys', 'PSV Eindhoven', 'FC Lorient', 'RasenBallsport Leipzig'],
  },
  {
    id: 398,
    name: 'Romain Faivre',
    country: 'Francja',
    clubs: ['Stade Brestois 29', 'FC Lorient', 'Olympique Lyon', 'AS Monaco'],
  },
  {
    id: 399,
    name: 'Siriné Doucouré',
    country: 'Francja',
    clubs: ['FC Lorient', 'LB Châteauroux'],
  },
  {
    id: 400,
    name: 'Folarin Balogun',
    country: 'USA',
    clubs: ['Stade Reims', 'FC Middlesbrough', 'FC Arsenal'],
  },
  {
    id: 401,
    name: 'Lucas Perrin',
    country: 'Francja',
    clubs: ['RC Strasbourg Alsace', 'Olympique Marseille'],
  },
  {
    id: 402,
    name: 'Colin Dagba',
    country: 'Francja',
    clubs: ['FC Paris Saint-Germain', 'RC Strasbourg Alsace'],
  },
  {
    id: 403,
    name: 'Frédéric Guilbert',
    country: 'Francja',
    clubs: ['SM Caen', 'RC Strasbourg Alsace', 'FC Girondins Bordeaux', 'Aston Villa'],
  },
  {
    id: 404,
    name: 'Bingourou Kamara',
    country: 'Senegal',
    clubs: ['Tours FC', 'RC Strasbourg Alsace', 'RSC Charleroi', 'Montpellier HSC'],
  },
  {
    id: 405,
    name: 'Kiki Kouyaté',
    country: 'Mali',
    clubs: ['FC Metz', 'ESTAC Troyes', 'Kawkab Marrakech', 'Montpellier HSC'],
  },
  {
    id: 406,
    name: 'Florent Mollet',
    country: 'Francja',
    clubs: ['Montpellier HSC', 'Dijon FCO', 'FC Metz', 'US Créteil-Lusitanos', 'FC Nantes', 'FC Schalke 04'],
  },
  {
    id: 407,
    name: 'Moussa Sissoko',
    country: 'Francja',
    clubs: ['FC Toulouse', 'Tottenham Hotspur', 'Newcastle United', 'FC Nantes', 'FC Watford'],
  },
  {
    id: 408,
    name: 'Memphis Depay',
    country: 'Holandia',
    clubs: ['Olympique Lyon', 'PSV Eindhoven', 'Manchester United', 'FC Barcelona', 'Atlético Madryt'],
  },
  {
    id: 409,
    name: 'Axel Witsel',
    country: 'Belgia',
    clubs: ['Standard Liège', 'Zenit Petersburg', 'Borussia Dortmund', 'Benfika Lizbona', 'Atlético Madryt'],
  },
  {
    id: 410,
    name: 'Matt Doherty',
    country: 'Irlandia',
    clubs: ['Wolverhampton Wanderers', 'Tottenham Hotspur', 'FC Bury', 'Hibernian FC', 'Atlético Madryt'],
  },
  {
    id: 411,
    name: 'Sergio Reguilón',
    country: 'Hiszpania',
    clubs: ['UD Logroñés', 'Tottenham Hotspur', 'Sevilla FC', 'Real Madryt', 'Atlético Madryt'],
  },
  {
    id: 412,
    name: 'Álvaro Morata',
    country: 'Hiszpania',
    clubs: ['Juventus Turyn', 'Chelsea FC', 'Real Madryt', 'Atlético Madryt'],
  },
  {
    id: 413,
    name: 'Saúl Ñíguez',
    country: 'Hiszpania',
    clubs: ['Rayo Vallecano', 'Chelsea FC', 'Atlético Madryt'],
  },
  {
    id: 414,
    name: 'Isco',
    country: 'Hiszpania',
    clubs: ['Málaga CF', 'Real Madryt', 'Sevilla FC', 'Valencia CF'],
  },
  {
    id: 415,
    name: 'Alex Telles',
    country: 'Brazylia',
    clubs: ['FC Porto', 'Galatasaray', 'Manchester United', 'Grêmio Porto Alegre', 'Sevilla FC', 'Inter Mediolan'],
  },
  {
    id: 416,
    name: 'Mats Hummels',
    country: 'Niemcy',
    clubs: ['Borussia Dortmund', 'Bayern Monachium'],
  },
  {
    id: 417,
    name: 'Leroy Sane',
    country: 'Niemcy',
    clubs: ['Manchester City', 'Bayern Monachium', 'FC Schalke 04'],
  },
  {
    id: 418,
    name: 'Serge Gnabry',
    country: 'Niemcy',
    clubs: ['SV Werder Bremen', 'Bayern Monachium', 'TSG 1899 Hoffenheim', 'FC Arsenal', 'West Bromwich Albion'],
  },
  {
    id: 419,
    name: 'Kingsley Coman',
    country: 'Francja',
    clubs: ['Bayern Monachium', 'Juventus Turyn', 'FC Paris Saint-Germain'],
  },
  {
    id: 420,
    name: 'Matthijs de Ligt',
    country: 'Holandia',
    clubs: ['Bayern Monachium', 'Juventus Turyn', 'Ajax Amsterdam'],
  },
  {
    id: 421,
    name: 'Dayot Upamecano',
    country: 'Francja',
    clubs: ['RasenBallsport Leipzig', 'Bayern Monachium', 'Red Bull Salzburg', 'FC Liefering'],
  },
  {
    id: 422,
    name: 'Yann Sommer',
    country: 'Szwajcaria',
    clubs: ['Borussia Mönchengladbach', 'FC Basel 1893', 'FC Vaduz', 'Grasshopper Club Zurych', 'Bayern Monachium'],
  },
  {
    id: 423,
    name: 'Davie Selke',
    country: 'Niemcy',
    clubs: ['Hertha BSC', 'SV Werder Bremen', 'RasenBallsport Leipzig', '1.FC Köln'],
  },
  {
    id: 424,
    name: 'Nadiem Amiri',
    country: 'Niemcy',
    clubs: ['Bayer 04 Leverkusen', 'TSG 1899 Hoffenheim', 'Genoa CFC'],
  },
  {
    id: 425,
    name: 'Tomasz Hajto',
    country: 'Polska',
    clubs: [
      'FC Schalke 04',
      'Górnik Zabrze',
      'MSV Duisburg',
      'ŁKS Łódź',
      'FC Southampton',
      '1.FC Nürnberg',
      'Derby County',
    ],
  },
  {
    id: 426,
    name: 'Gonzalo Higuaín',
    country: 'Argentyna',
    clubs: [
      'Real Madryt',
      'Juventus Turyn',
      'SSC Napoli',
      'Inter Miami CF',
      'CA River Plate',
      'AC Milan',
      'Chelsea FC',
    ],
  },
  {
    id: 427,
    name: 'Zlatan Ibrahimović',
    country: 'Szwecja',
    clubs: [
      'FC Paris Saint-Germain',
      'AC Milan',
      'Inter Mediolan',
      'Ajax Amsterdam',
      'Juventus Turyn',
      'Los Angeles Galaxy',
      'Manchester United',
      'FC Barcelona',
      'Malmö FF',
    ],
  },
  {
    id: 428,
    name: 'Olivier Giroud',
    country: 'Francja',
    clubs: [
      'FC Arsenal',
      'Chelsea FC',
      'AC Milan',
      'Montpellier HSC',
      'Tours FC',
      'Istres Football Club',
      'Grenoble Foot 38',
    ],
  },
  {
    id: 429,
    name: 'Sergiño Dest',
    country: 'USA',
    clubs: ['FC Barcelona', 'Ajax Amsterdam', 'AC Milan'],
  },
  {
    id: 430,
    name: 'Leonardo Bonucci',
    country: 'Włochy',
    clubs: ['Juventus Turyn', 'AC Milan', 'Treviso FBC 1993', 'SSC Bari', 'Pisa Sporting Club', 'Inter Mediolan'],
  },
  {
    id: 431,
    name: 'Ronaldinho',
    country: 'Brazylia',
    clubs: ['FC Barcelona', 'AC Milan', 'FC Paris Saint-Germain', 'Flamengo Rio de Janeiro'],
  },
  {
    id: 432,
    name: 'Mario Balotelli',
    country: 'Włochy',
    clubs: [
      'Inter Mediolan',
      'AC Milan',
      'Manchester City',
      'OGC Nice',
      'FC Liverpool',
      'Brescia Calcio',
      'FC Sion',
      'Olympique Marseille',
      'AC Monza',
    ],
  },
  {
    id: 433,
    name: 'Sadio Mané',
    country: 'Senegal',
    clubs: ['FC Liverpool', 'Red Bull Salzburg', 'FC Southampton', 'Bayern Monachium', 'FC Metz'],
  },
  {
    id: 434,
    name: 'Gareth Bale',
    country: 'Walia',
    clubs: ['Real Madryt', 'Tottenham Hotspur', 'FC Southampton', 'Los Angeles FC'],
  },
  {
    id: 435,
    name: 'Theo Walcott',
    country: 'Anglia',
    clubs: ['FC Arsenal', 'FC Everton', 'FC Southampton'],
  },
  {
    id: 436,
    name: 'Roberto Firmino',
    country: 'Brayzlia',
    clubs: ['FC Liverpool', 'TSG 1899 Hoffenheim', 'Figueirense Futebol Clube'],
  },
  {
    id: 437,
    name: 'Eduardo Vargas',
    country: 'Chile',
    clubs: [
      'Tigres UANL',
      'Atlético Mineiro',
      'TSG 1899 Hoffenheim',
      'SSC Napoli',
      'Valencia CF',
      'Queens Park Rangers',
    ],
  },
  {
    id: 438,
    name: 'Giovani Lo Celso',
    country: 'Argentyna',
    clubs: [
      'Tottenham Hotspur',
      'FC Paris Saint-Germain',
      'CA Rosario Central',
      'Villarreal CF',
      'Real Betis Balompié',
    ],
  },
  {
    id: 439,
    name: 'Junior Firpo',
    country: 'Hiszpania',
    clubs: ['Leeds United', 'Real Betis Balompié', 'FC Barcelona'],
  },
  {
    id: 440,
    name: 'Dani Ceballos',
    country: 'Hiszpania',
    clubs: ['Real Madryt', 'Real Betis Balompié', 'FC Arsenal'],
  },
  {
    id: 441,
    name: 'Dani Pacheco',
    country: 'Hiszpania',
    clubs: [
      'Getafe CF',
      'Málaga CF',
      'Górnik Zabrze',
      'Deportivo Alavés',
      'AD Alcorcón',
      'Real Betis Balompié',
      'SD Huesca',
      'FC Liverpool',
      'Aris Limassol',
      'Rayo Vallecano',
      'UD Logroñés',
      'Norwich City',
    ],
  },
  {
    id: 442,
    name: 'Erling Haaland',
    country: 'Norwegia',
    clubs: ['Borussia Dortmund', 'Manchester City', 'Molde FK', 'Red Bull Salzburg'],
  },
  {
    id: 443,
    name: 'Enzo Fernández',
    country: 'Argentyna',
    clubs: ['CA River Plate', 'Benfika Lizbona', 'Chelsea FC'],
  },
  {
    id: 444,
    name: 'Casemiro',
    country: 'Brazylia',
    clubs: ['Real Madryt', 'FC Sao Paulo', 'Manchester United', 'FC Porto'],
  },
  {
    id: 445,
    name: 'Raheem Sterling',
    country: 'Anglia',
    clubs: ['Manchester City', 'FC Liverpool', 'Chelsea FC'],
  },
  {
    id: 446,
    name: 'Romelu Lukaku',
    country: 'Belgia',
    clubs: [
      'FC Everton',
      'Inter Mediolan',
      'RSC Anderlecht',
      'Manchester United',
      'Chelsea FC',
      'West Bromwich Albion',
    ],
  },
  {
    id: 447,
    name: 'João Cancelo',
    country: 'Portugalia',
    clubs: [
      'Manchester City',
      'Valencia CF',
      'Juventus Turyn',
      'Inter Mediolan',
      'Bayern Monachium',
      'Benfika Lizbona',
    ],
  },
  {
    id: 448,
    name: 'Cody Gakpo',
    country: 'Holandia',
    clubs: ['PSV Eindhoven', 'FC Liverpool'],
  },
  {
    id: 449,
    name: 'Raphinha',
    country: 'Brazylia',
    clubs: ['Vitoria Guimarães SC', 'Leeds United', 'FC Barcelona', 'Sporting CP', 'Stade Rennais FC'],
  },
  {
    id: 450,
    name: 'Gabriel Jesus',
    country: 'Brazylia',
    clubs: ['Manchester City', 'Sociedade Esportiva Palmeiras', 'FC Arsenal'],
  },
  {
    id: 451,
    name: 'Anthony Gordon',
    country: 'Anglia',
    clubs: ['FC Everton', 'Newcastle United', 'Preston North End'],
  },
  {
    id: 452,
    name: 'Diego Carlos',
    country: 'Brazylia',
    clubs: ['Sevilla FC', 'FC Nantes', 'Aston Villa'],
  },
  {
    id: 453,
    name: 'Gonçalo Guedes',
    country: 'Portugalia',
    clubs: ['Valencia CF', 'Benfika Lizbona', 'Wolverhampton Wanderers', 'FC Paris Saint-Germain'],
  },
  {
    id: 454,
    name: 'Brenden Aaronson',
    country: 'USA',
    clubs: ['Red Bull Salzburg', 'Philadelphia Union', 'Leeds United'],
  },
  {
    id: 455,
    name: 'Paul Pogba',
    country: 'Francja',
    clubs: ['Manchester United', 'Juventus Turyn'],
  },
  {
    id: 456,
    name: 'Paulo Dybala',
    country: 'Argentyna',
    clubs: ['Palermo FC', 'Juventus Turyn', 'AS Roma'],
  },
  {
    id: 457,
    name: 'Nicolò Zaniolo',
    country: 'Włochy',
    clubs: ['Galatasaray', 'AS Roma'],
  },
  {
    id: 458,
    name: 'Luka Jovic',
    country: 'Serbia',
    clubs: ['Eintracht Frankfurt', 'Real Madryt', 'AC Fiorentina', 'Crvena Zvezda Belgrad'],
  },
  {
    id: 459,
    name: 'Ricardo Quaresma',
    country: 'Portugalia',
    clubs: [
      'Besiktas JK',
      'FC Porto',
      'Sporting CP',
      'Vitoria Guimarães SC',
      'Inter Mediolan',
      'FC Barcelona',
      'Chelsea FC',
    ],
  },
  {
    id: 460,
    name: 'Tim Borowski',
    country: 'Niemcy',
    clubs: ['SV Werder Bremen', 'Bayern Monachium'],
  },
  {
    id: 461,
    name: 'Nuno Gomes',
    country: 'Portugalia',
    clubs: ['Benfika Lizbona', 'AC Fiorentina', 'SC Braga', 'Blackburn Rovers'],
  },
  {
    id: 462,
    name: 'Ariel Ortega',
    country: 'Argentyna',
    clubs: ['CA River Plate', 'Valencia CF', 'UC Sampdoria', 'Parma Calcio 1913', 'Fenerbahce'],
  },
  {
    id: 463,
    name: 'Robbie Keane',
    country: 'Irlandia',
    clubs: [
      'Tottenham Hotspur',
      'Los Angeles Galaxy',
      'Wolverhampton Wanderers',
      'Leeds United',
      'Coventry City',
      'FC Liverpool',
      'Celtic Glasgow',
      'Inter Mediolan',
      'West Ham United',
      'Aston Villa',
    ],
  },
  {
    id: 464,
    name: 'Jermaine Pennant',
    country: 'Anglia',
    clubs: [
      'Stoke City',
      'FC Liverpool',
      'Birmingham City',
      'Leeds United',
      'Tampines Rovers',
      'Real Zaragoza',
      'FC Arsenal',
      'FC Watford',
      'Wolverhampton Wanderers',
      'FC Portsmouth',
      'Wigan Athletic',
    ],
  },
  {
    id: 465,
    name: 'Peter Crouch',
    country: 'Anglia',
    clubs: [
      'Stoke City',
      'FC Liverpool',
      'Tottenham Hotspur',
      'FC Portsmouth',
      'Queens Park Rangers',
      'Aston Villa',
      'FC Southampton',
      'Norwich City',
      'FC Burnley',
    ],
  },
  {
    id: 466,
    name: 'Marcus Bent',
    country: 'Anglia',
    clubs: [
      'Ipswich Town',
      'FC Everton',
      'Sheffield United',
      'Charlton Athletic',
      'Blackburn Rovers',
      'Birmingham City',
      'Leicester City',
      'Wigan Athletic',
      'Crystal Palace',
      'FC Middlesbrough',
      'Wolverhampton Wanderers',
      'Queens Park Rangers',
      'FC Brentford',
    ],
  },
  {
    id: 467,
    name: 'Andy Cole',
    country: 'Anglia',
    clubs: [
      'Manchester United',
      'Blackburn Rovers',
      'Newcastle United',
      'FC Fulham',
      'Manchester City',
      'FC Portsmouth',
      'FC Burnley',
      'Nottingham Forest',
      'AFC Sunderland',
      'Birmingham City',
      'FC Arsenal',
      'Bristol City',
    ],
  },
  {
    id: 468,
    name: 'Ciro Immobile',
    country: 'Włochy',
    clubs: ['Lazio Rzym', 'Torino FC', 'Borussia Dortmund', 'Genoa CFC', 'Sevilla FC', 'Juventus Turyn'],
  },
  {
    id: 469,
    name: 'Luca Pellegrini',
    country: 'Włochy',
    clubs: ['Cagliari Calcio', 'Juventus Turyn', 'Eintracht Frankfurt', 'Genoa CFC', 'Lazio Rzym', 'AS Roma'],
  },
  {
    id: 470,
    name: 'Andrea Belotti',
    country: 'Włochy',
    clubs: ['Torino FC', 'Palermo FC', 'AS Roma'],
  },
  {
    id: 471,
    name: 'Sébastien Haller',
    country: 'Wybrzeże Kości Słoniowej',
    clubs: [
      'FC Utrecht',
      'Eintracht Frankfurt',
      'Ajax Amsterdam',
      'AJ Auxerre',
      'West Ham United',
      'Borussia Dortmund',
    ],
  },
  {
    id: 472,
    name: 'Chicharito',
    country: 'Meksyk',
    clubs: [
      'Manchester United',
      'Los Angeles Galaxy',
      'Deportivo Guadalajara',
      'Bayer 04 Leverkusen',
      'West Ham United',
      'Real Madryt',
      'Sevilla FC',
    ],
  },
  {
    id: 473,
    name: 'Carlos Tevez',
    country: 'Argentyna',
    clubs: [
      'CA Boca Juniors',
      'Manchester City',
      'Manchester United',
      'West Ham United',
      'Juventus Turyn',
      'Shanghai Shenhua',
    ],
  },
  {
    id: 474,
    name: 'Robert Gumny',
    country: 'Polska',
    clubs: ['Lech Poznań', 'FC Augsburg', 'Podbeskidzie Bielsko-Biała'],
  },
  {
    id: 475,
    name: 'Cristiano Ronaldo',
    country: 'Portugalia',
    clubs: ['Real Madryt', 'Manchester United', 'Juventus Turyn', 'Sporting CP', 'Al-Nassr FC'],
  },
  {
    id: 476,
    name: 'Eden Hazard',
    country: 'Belgia',
    clubs: ['Chelsea FC', 'LOSC Lille', 'Real Madryt'],
  },
  {
    id: 477,
    name: 'James Rodríguez',
    country: 'Kolumbia',
    clubs: ['Bayern Monachium', 'FC Porto', 'Real Madryt', 'AS Monaco', 'FC Everton', 'Olympiakos Pireus'],
  },
  {
    id: 478,
    name: 'Thibaut Courtois',
    country: 'Belgia',
    clubs: ['Atlético Madryt', 'Chelsea FC', 'Real Madryt', 'KRC Genk'],
  },
  {
    id: 479,
    name: 'Tyrone Mings',
    country: 'Anglia',
    clubs: ['Aston Villa', 'Ipswich Town', 'AFC Bournemouth'],
  },
  {
    id: 480,
    name: 'Kieron Dyer',
    country: 'Anglia',
    clubs: ['Newcastle United', 'Ipswich Town', 'West Ham United', 'FC Middlesbrough', 'Queens Park Rangers'],
  },
  {
    id: 481,
    name: 'Harry Clarke',
    country: 'Anglia',
    clubs: ['Oldham Athletic', 'Ipswich Town', 'Stoke City', 'Ross County FC'],
  },
  {
    id: 482,
    name: 'Michal Koj',
    country: 'Polska',
    clubs: ['Górnik Zabrze', 'Ruch Chorzów', 'Korona Kielce', 'Puszcza Niepołomice'],
  },
  {
    id: 483,
    name: 'Thiago',
    country: 'Brazylia',
    clubs: ['Cracovia', 'Sandecja Nowy Sącz', 'Puszcza Niepołomice'],
  },
  {
    id: 484,
    name: 'Artur Siemaszko',
    country: 'Polska',
    clubs: ['Stomil Olsztyn', 'Arka Gdynia', 'Puszcza Niepołomice', 'GKS Tychy', 'Zagłębie Lubin'],
  },
  {
    id: 485,
    name: 'Kamil Zapolnik',
    country: 'Polska',
    clubs: ['Miedź Legnica', 'Wigry Suwałki', 'Górnik Zabrze', 'Olimpia Zambrów', 'GKS Tychy', 'Puszcza Niepołomice'],
  },
  {
    id: 486,
    name: 'Rafal Boguski',
    country: 'Polska',
    clubs: ['Wisła Kraków', 'GKS Bełchatów', 'Puszcza Niepołomice'],
  },
  {
    id: 487,
    name: 'Dominik Frelek',
    country: 'Polska',
    clubs: ['Podbeskidzie Bielsko-Biała', 'Olimpia Grudziądz', 'Puszcza Niepołomice'],
  },
  {
    id: 488,
    name: 'Maciej Domanski',
    country: 'Polska',
    clubs: ['Stal Mielec', 'Raków Częstochowa', 'Puszcza Niepołomice', 'Siarka Tarnobrzeg'],
  },
  {
    id: 489,
    name: 'Pawel Moskwik',
    country: 'Polska',
    clubs: ['Znicz Pruszków', 'Motor Lublin', 'Puszcza Niepołomice', 'Piast Gliwice', 'Podbeskidzie Bielsko-Biała'],
  },
  {
    id: 490,
    name: 'Roland Sallai',
    country: 'Węgry',
    clubs: ['SC Freiburg', 'Puskás Akadémia FC', 'APOEL Nikosia', 'Palermo FC'],
  },
  {
    id: 491,
    name: 'Pascal Stenzel',
    country: 'Niemcy',
    clubs: ['VfB Stuttgart', 'SC Freiburg', 'Borussia Dortmund'],
  },
  {
    id: 492,
    name: 'Marco Terrazzino',
    country: 'Niemcy',
    clubs: [
      'VfL Bochum',
      'SC Freiburg',
      'Lechia Gdańsk',
      'TSG 1899 Hoffenheim',
      'SC Paderborn 07',
      'SG Dynamo Dresden',
    ],
  },
  {
    id: 493,
    name: 'Caglar Söyüncü',
    country: 'Turcja',
    clubs: ['Leicester City', 'SC Freiburg', 'Altinordu FK'],
  },
  {
    id: 494,
    name: 'Baptiste Santamaria',
    country: 'Francja',
    clubs: ['Angers SCO', 'SC Freiburg', 'Tours FC', 'Stade Rennais FC'],
  },
  {
    id: 495,
    name: 'Robin Koch',
    country: 'Niemcy',
    clubs: ['Leeds United', 'SC Freiburg', '1.FC Kaiserslautern', 'SV Eintracht Trier 05'],
  },
  {
    id: 496,
    name: 'Papiss Cissé',
    country: 'Senegal',
    clubs: ['Newcastle United', 'SC Freiburg', 'FC Metz', 'Alanyaspor', 'Shandong Taishan', 'Amiens SC', 'Fenerbahce'],
  },
  {
    id: 497,
    name: 'Oliver Baumann',
    country: 'Niemcy',
    clubs: ['TSG 1899 Hoffenheim', 'SC Freiburg'],
  },
  {
    id: 498,
    name: 'Roman Bürki',
    country: 'Szwajcaria',
    clubs: [
      'Borussia Dortmund',
      'SC Freiburg',
      'Grasshopper Club Zurych',
      'St. Louis CITY SC',
      'FC Schaffhausen',
      'BSC Young Boys',
      'FC Thun',
    ],
  },
  {
    id: 499,
    name: 'Ariel Borysiuk',
    country: 'Polska',
    clubs: [
      'Legia Warszawa',
      'Lechia Gdańsk',
      '1.FC Kaiserslautern',
      'Jagiellonia Białystok',
      'KF Laçi',
      'Chennaiyin FC',
      'Queens Park Rangers',
      'FC Sheriff Tyraspol',
      'Wisła Płock',
    ],
  },
  {
    id: 500,
    name: 'Ugo Ukah',
    country: 'Nigeria',
    clubs: ['Widzew Łódź', 'Jagiellonia Białystok', 'AEL Kalloni', 'Selangor FC', 'Queens Park Rangers'],
  },
  {
    id: 501,
    name: 'Pawel Kryszalowicz',
    country: 'Polska',
    clubs: ['Amica Wronki', 'Eintracht Frankfurt', 'Wisła Kraków', 'SV Wilhelmshaven'],
  },
  {
    id: 502,
    name: 'Pablo Sarabia',
    country: 'Hiszpania',
    clubs: [
      'Sevilla FC',
      'Getafe CF',
      'FC Paris Saint-Germain',
      'Sporting CP',
      'Wolverhampton Wanderers',
      'Real Madryt',
    ],
  },
  {
    id: 503,
    name: 'Rúben Neves',
    country: 'Portugalia',
    clubs: ['Wolverhampton Wanderers', 'FC Porto'],
  },
  {
    id: 504,
    name: 'Nicolás Otamendi',
    country: 'Argentyna',
    clubs: ['Manchester City', 'Benfika Lizbona', 'FC Porto', 'CA Vélez Sarsfield', 'Valencia CF', 'Atlético Mineiro'],
  },
  {
    id: 505,
    name: 'Felipe Anderson',
    country: 'Brazylia',
    clubs: ['Lazio Rzym', 'West Ham United', 'FC Porto', 'Santos FC'],
  },
  {
    id: 506,
    name: 'Alphonse Areola',
    country: 'Francja',
    clubs: [
      'FC Paris Saint-Germain',
      'West Ham United',
      'SC Bastia',
      'Villarreal CF',
      'FC Fulham',
      'RC Lens',
      'Real Madryt',
    ],
  },
  {
    id: 507,
    name: 'Lucas Leiva',
    country: 'Brazylia',
    clubs: ['FC Liverpool', 'Lazio Rzym', 'Grêmio Porto Alegre'],
  },
  {
    id: 508,
    name: 'Felipe Caicedo',
    country: 'Ekwador',
    clubs: [
      'Lazio Rzym',
      'RCD Espanyol',
      'Lokomotiw Moskwa',
      'FC Basel 1893',
      'Manchester City',
      'Levante UD',
      'Málaga CF',
      'Sporting CP',
      'Genoa CFC',
      'Inter Mediolan',
    ],
  },
  {
    id: 509,
    name: 'Émerson',
    country: 'Brazylia',
    clubs: ['AS Roma', 'Bayer 04 Leverkusen', 'Juventus Turyn', 'AC Milan', 'Real Madryt'],
  },
  {
    id: 510,
    name: 'Ronaldo',
    country: 'Brazylia',
    clubs: ['Real Madryt', 'Inter Mediolan', 'PSV Eindhoven', 'FC Barcelona', 'AC Milan'],
  },
  {
    id: 511,
    name: 'Radamel Falcao',
    country: 'Kolumbia',
    clubs: [
      'AS Monaco',
      'Atlético Madryt',
      'FC Porto',
      'Rayo Vallecano',
      'Galatasaray',
      'Manchester United',
      'Chelsea FC',
    ],
  },
  {
    id: 512,
    name: 'Klaas-Jan Huntelaar',
    country: 'Holandia',
    clubs: [
      'Ajax Amsterdam',
      'FC Schalke 04',
      'SC Heerenveen',
      'AC Milan',
      'Real Madryt',
      'De Graafschap Doetinchem',
      'PSV Eindhoven',
    ],
  },
  {
    id: 513,
    name: 'Bernardo Silva',
    country: 'Portugalia',
    clubs: ['Manchester City', 'AS Monaco', 'Benfika Lizbona'],
  },
  {
    id: 514,
    name: 'Harry Kane',
    country: 'Anglia',
    clubs: ['Tottenham Hotspur', 'FC Millwall', 'Leicester City', 'Norwich City'],
  },
  {
    id: 515,
    name: 'Robinho',
    country: 'Brazylia',
    clubs: ['AC Milan', 'Real Madryt', 'Manchester City', 'Basaksehir FK', 'Sivasspor'],
  },
  {
    id: 516,
    name: 'Frank Lampard',
    country: 'Anglia',
    clubs: ['Chelsea FC', 'West Ham United', 'Manchester City'],
  },
  {
    id: 517,
    name: 'Gerard Piqué',
    country: 'Hiszpania',
    clubs: ['FC Barcelona', 'Real Zaragoza', 'Manchester United'],
  },
  {
    id: 518,
    name: 'Cesc Fàbregas',
    country: 'Hiszpania',
    clubs: ['FC Arsenal', 'Chelsea FC', 'FC Barcelona', 'AS Monaco'],
  },
  {
    id: 519,
    name: 'Pierre-Emerick Aubameyang',
    country: 'Gabon',
    clubs: [
      'FC Arsenal',
      'Borussia Dortmund',
      'AS Saint-Étienne',
      'Dijon FCO',
      'LOSC Lille',
      'FC Barcelona',
      'AS Monaco',
      'Chelsea FC',
    ],
  },
  {
    id: 520,
    name: 'Robin van Persie',
    country: 'Holandia',
    clubs: ['FC Arsenal', 'Feyenoord Rotterdam', 'Manchester United', 'Fenerbahce'],
  },
  {
    id: 521,
    name: 'Ruud van Nistelrooy',
    country: 'Holandia',
    clubs: ['PSV Eindhoven', 'Real Madryt', 'Manchester United', 'Hamburger SV', 'SC Heerenveen', 'Málaga CF'],
  },
  {
    id: 522,
    name: 'Wesley Sneijder',
    country: 'Holandia',
    clubs: ['Ajax Amsterdam', 'Galatasaray', 'Inter Mediolan', 'Real Madryt', 'OGC Nice'],
  },
  {
    id: 523,
    name: 'Andrea Pirlo',
    country: 'Włochy',
    clubs: ['AC Milan', 'Juventus Turyn', 'New York City FC', 'Brescia Calcio', 'Inter Mediolan'],
  },
  {
    id: 524,
    name: 'Fabio Cannavaro',
    country: 'Włochy',
    clubs: ['Parma Calcio 1913', 'Juventus Turyn', 'Real Madryt', 'SSC Napoli', 'Inter Mediolan'],
  },
  {
    id: 525,
    name: 'Marko Arnautovic',
    country: 'Austria',
    clubs: ['Stoke City', 'SV Werder Bremen', 'West Ham United', 'FC Twente Enschede', 'FC Bologna', 'Inter Mediolan'],
  },
  {
    id: 526,
    name: 'Yaya Touré',
    country: 'Wybrzeże Kości Słoniowej',
    clubs: ['Manchester City', 'FC Barcelona', 'Olympiakos Pireus', 'AS Monaco'],
  },
  {
    id: 527,
    name: 'Kolo Touré',
    country: 'Wybrzeże Kości Słoniowej',
    clubs: ['FC Arsenal', 'Manchester City', 'FC Liverpool', 'Celtic Glasgow'],
  },
  {
    id: 528,
    name: 'Lukas Hradecky',
    country: 'Finlandia',
    clubs: ['Bayer 04 Leverkusen', 'Eintracht Frankfurt', 'Esbjerg fB', 'Bröndby IF'],
  },
  {
    id: 529,
    name: 'Jonathan Tah',
    country: 'Niemcy',
    clubs: ['Bayer 04 Leverkusen', 'Fortuna Düsseldorf', 'Hamburger SV'],
  },
  {
    id: 530,
    name: 'Patrik Schick',
    country: 'Czechy',
    clubs: [
      'Bayer 04 Leverkusen',
      'AS Roma',
      'UC Sampdoria',
      'Bohemians Praga 1905',
      'AC Sparta Praga',
      'RasenBallsport Leipzig',
    ],
  },
  {
    id: 531,
    name: 'Leon Bailey',
    country: 'Jamajka',
    clubs: ['Bayer 04 Leverkusen', 'KRC Genk', 'Aston Villa'],
  },
  {
    id: 532,
    name: 'Heung-min Son',
    country: 'Korea Południowa',
    clubs: ['Bayer 04 Leverkusen', 'Tottenham Hotspur', 'Hamburger SV'],
  },
  {
    id: 533,
    name: 'Kai Havertz',
    country: 'Niemcy',
    clubs: ['Bayer 04 Leverkusen', 'Chelsea FC'],
  },
  {
    id: 534,
    name: 'Hakan Calhanoglu',
    country: 'Turcja',
    clubs: ['AC Milan', 'Bayer 04 Leverkusen', 'Inter Mediolan', 'Hamburger SV'],
  },
  {
    id: 535,
    name: 'Arturo Vidal',
    country: 'Chile',
    clubs: ['Juventus Turyn', 'Bayer 04 Leverkusen', 'Bayern Monachium', 'FC Barcelona', 'Inter Mediolan'],
  },
  {
    id: 536,
    name: 'Emre Can',
    country: 'Niemcy',
    clubs: ['FC Liverpool', 'Bayer 04 Leverkusen', 'Borussia Dortmund', 'Juventus Turyn', 'Bayern Monachium'],
  },
  {
    id: 537,
    name: 'Kamil Vacek',
    country: 'Czechy',
    clubs: [
      'AC Sparta Praga',
      'Bohemians Praga 1905',
      'Chievo Verona',
      'Piast Gliwice',
      'Śląsk Wrocław',
      'Arminia Bielefeld',
    ],
  },
  {
    id: 538,
    name: 'Francesco Acerbi',
    country: 'Włochy',
    clubs: [
      'Lazio Rzym',
      'US Sassuolo',
      'Chievo Verona',
      'Inter Mediolan',
      'Reggina 1914',
      'Spezia Primavera',
      'AC Milan',
    ],
  },
  {
    id: 539,
    name: 'Ismaïla Sarr',
    country: 'Senegal',
    clubs: ['FC Watford', 'Stade Rennais FC', 'FC Metz'],
  },
  {
    id: 540,
    name: 'Franck Ribéry',
    country: 'Francja',
    clubs: ['Bayern Monachium', 'Olympique Marseille', 'AC Fiorentina', 'FC Metz', 'Galatasaray'],
  },
  {
    id: 541,
    name: 'Jacek Goralski',
    country: 'Polska',
    clubs: ['Wisła Płock', 'Ludogorets Razgrad', 'Jagiellonia Białystok', 'Kairat Almaty', 'VfL Bochum'],
  },
  {
    id: 542,
    name: 'Júlio Baptista',
    country: 'Brazylia',
    clubs: ['Sevilla FC', 'Real Madryt', 'AS Roma', 'FC Arsenal', 'Málaga CF'],
  },
  {
    id: 543,
    name: 'Santi Cazorla',
    country: 'Hiszpania',
    clubs: ['Villarreal CF', 'FC Arsenal', 'Málaga CF'],
  },
  {
    id: 544,
    name: 'Edwin van der Sar',
    country: 'Holandia',
    clubs: ['Ajax Amsterdam', 'Manchester United', 'FC Fulham', 'Juventus Turyn'],
  },
  {
    id: 545,
    name: 'Louis Saha',
    country: 'Francja',
    clubs: [
      'FC Fulham',
      'Manchester United',
      'FC Everton',
      'FC Metz',
      'AFC Sunderland',
      'Tottenham Hotspur',
      'Newcastle United',
      'Lazio Rzym',
    ],
  },
  {
    id: 546,
    name: 'Fábio Carvalho',
    country: 'Portugalia',
    clubs: ['FC Fulham', 'FC Liverpool'],
  },
  {
    id: 547,
    name: 'Max Kruse',
    country: 'Niemcy',
    clubs: [
      'FC St. Pauli',
      'SV Werder Bremen',
      'Borussia Mönchengladbach',
      'VfL Wolfsburg',
      '1.FC Union Berlin',
      'SC Freiburg',
      'Fenerbahce',
    ],
  },
  {
    id: 548,
    name: 'Andrzej Juskowiak',
    country: 'Polska',
    clubs: [
      'VfL Wolfsburg',
      'Olympiakos Pireus',
      'Borussia Mönchengladbach',
      'Lech Poznań',
      'Sporting CP',
      'New York Red Bulls',
    ],
  },
  {
    id: 549,
    name: 'Thorgan Hazard',
    country: 'Belgia',
    clubs: ['Borussia Mönchengladbach', 'Borussia Dortmund', 'SV Zulte Waregem', 'RC Lens', 'PSV Eindhoven'],
  },
  {
    id: 550,
    name: 'Granit Xhaka',
    country: 'Szwajcaria',
    clubs: ['Borussia Mönchengladbach', 'FC Arsenal', 'FC Basel 1893'],
  },
  {
    id: 551,
    name: 'Luuk de Jong',
    country: 'Holandia',
    clubs: [
      'PSV Eindhoven',
      'FC Twente Enschede',
      'Sevilla FC',
      'Borussia Mönchengladbach',
      'FC Barcelona',
      'De Graafschap Doetinchem',
      'Newcastle United',
    ],
  },
  {
    id: 552,
    name: 'Christoph Kramer',
    country: 'Niemcy',
    clubs: ['Borussia Mönchengladbach', 'VfL Bochum', 'Bayer 04 Leverkusen'],
  },
  {
    id: 553,
    name: 'Luís Figo',
    country: 'Portugalia',
    clubs: ['FC Barcelona', 'Real Madryt', 'Inter Mediolan', 'Sporting CP'],
  },
  {
    id: 554,
    name: 'Timothée Kolodziejczak',
    country: 'Francja',
    clubs: [
      'AS Saint-Étienne',
      'Sevilla FC',
      'OGC Nice',
      'Olympique Lyon',
      'Borussia Mönchengladbach',
      'FC Schalke 04',
    ],
  },
  {
    id: 555,
    name: 'Yevgen Konoplyanka',
    country: 'Ukraina',
    clubs: ['Dnipro Dniepropetrowsk', 'Sevilla FC', 'FC Schalke 04', 'Szachtar Donieck', 'Cracovia'],
  },
  {
    id: 556,
    name: 'Patrice Evra',
    country: 'Francja',
    clubs: [
      'Manchester United',
      'AS Monaco',
      'Juventus Turyn',
      'OGC Nice',
      'Olympique Marseille',
      'AC Monza',
      'West Ham United',
    ],
  },
  {
    id: 557,
    name: 'Allan Saint-Maximin',
    country: 'Francja',
    clubs: ['Newcastle United', 'OGC Nice', 'SC Bastia', 'Hannover 96', 'AS Saint-Étienne', 'AS Monaco'],
  },
  {
    id: 558,
    name: 'Daniel Osvaldo',
    country: 'Włochy',
    clubs: [
      'AS Roma',
      'RCD Espanyol',
      'AC Fiorentina',
      'US Lecce',
      'FC Bologna',
      'Inter Mediolan',
      'Juventus Turyn',
      'FC Southampton',
      'FC Porto',
      'Atalanta BC',
    ],
  },
  {
    id: 559,
    name: 'Ivan Perisic',
    country: 'Chorwacja',
    clubs: [
      'Inter Mediolan',
      'FC Brügge',
      'VfL Wolfsburg',
      'Borussia Dortmund',
      'Tottenham Hotspur',
      'Bayern Monachium',
    ],
  },
  {
    id: 560,
    name: 'Henrikh Mkhitaryan',
    country: 'Armenia',
    clubs: ['Inter Mediolan', 'Szachtar Donieck', 'AS Roma', 'Borussia Dortmund', 'Manchester United', 'FC Arsenal'],
  },
  {
    id: 561,
    name: 'Paulo Sousa',
    country: 'Portugalia',
    clubs: [
      'Juventus Turyn',
      'Inter Mediolan',
      'Panathinaikos Ateny',
      'Borussia Dortmund',
      'Benfika Lizbona',
      'Parma Calcio 1913',
      'RCD Espanyol',
      'Sporting CP',
    ],
  },
  {
    id: 562,
    name: 'Jan Koller',
    country: 'Czechy',
    clubs: ['AC Sparta Praga', 'AS Monaco', 'RSC Anderlecht', 'Borussia Dortmund', '1.FC Nürnberg'],
  },
  {
    id: 563,
    name: 'Abdoulaye Doucouré',
    country: 'Mali',
    clubs: ['FC Watford', 'FC Everton', 'Stade Rennais FC', 'Granada CF'],
  },
  {
    id: 564,
    name: 'Dalbert',
    country: 'Brazylia',
    clubs: ['OGC Nice', 'AC Fiorentina', 'Cagliari Calcio', 'Inter Mediolan', 'Stade Rennais FC'],
  },
  {
    id: 565,
    name: 'Alfonso Pedraza',
    country: 'Hiszpania',
    clubs: ['Villarreal CF', 'Deportivo Alavés', 'Real Betis Balompié', 'Leeds United'],
  },
  {
    id: 566,
    name: 'Petr Cech',
    country: 'Czechy',
    clubs: ['Chelsea FC', 'FC Arsenal', 'Stade Rennais FC', 'AC Sparta Praga'],
  },
  {
    id: 567,
    name: 'Jadon Sancho',
    country: 'Anglia',
    clubs: ['Borussia Dortmund', 'Manchester United'],
  },
  {
    id: 568,
    name: 'Lionel Messi',
    country: 'Argentyna',
    clubs: ['FC Barcelona', 'FC Paris Saint-Germain'],
  },
  {
    id: 569,
    name: 'Antoine Griezmann',
    country: 'Francja',
    clubs: ['FC Barcelona', 'Atlético Madryt', 'Real Sociedad'],
  },
  {
    id: 570,
    name: 'Raphaël Varane',
    country: 'Francja',
    clubs: ['Real Madryt', 'Manchester United', 'RC Lens'],
  },
  {
    id: 571,
    name: 'Ferran Torres',
    country: 'Hiszpania',
    clubs: ['Valencia CF', 'FC Barcelona', 'Manchester City'],
  },
  {
    id: 572,
    name: 'David Alaba',
    country: 'Austria',
    clubs: ['Bayern Monachium', 'Real Madryt', 'TSG 1899 Hoffenheim', 'FK Austria Wiedeń'],
  },
  {
    id: 573,
    name: 'Luis Díaz',
    country: 'Kolumbia',
    clubs: ['FC Porto', 'FC Liverpool'],
  },
  {
    id: 574,
    name: 'Ibrahima Konaté',
    country: 'Francja',
    clubs: ['RasenBallsport Leipzig', 'FC Liverpool'],
  },
  {
    id: 575,
    name: 'Martin Ødegaard',
    country: 'Norwegia',
    clubs: ['FC Arsenal', 'SC Heerenveen', 'Real Sociedad', 'Real Madryt'],
  },
  {
    id: 576,
    name: 'Sergio Agüero',
    country: 'Argentyna',
    clubs: ['Manchester City', 'Atlético Madryt', 'FC Barcelona'],
  },
  {
    id: 577,
    name: 'Juan Mata',
    country: 'Hiszpania',
    clubs: ['Manchester United', 'Valencia CF', 'Chelsea FC', 'Galatasaray'],
  },
  {
    id: 578,
    name: 'Stephan El Shaarawy',
    country: 'Włochy',
    clubs: ['AS Roma', 'AC Milan', 'Calcio Padova', 'AS Monaco', 'Genoa CFC'],
  },
  {
    id: 579,
    name: 'Nuri Sahin',
    country: 'Turcja',
    clubs: [
      'Borussia Dortmund',
      'Antalyaspor',
      'SV Werder Bremen',
      'Feyenoord Rotterdam',
      'FC Liverpool',
      'Real Madryt',
    ],
  },
  {
    id: 580,
    name: 'Emmanuel Adebayor',
    country: 'Togo',
    clubs: [
      'FC Arsenal',
      'AS Monaco',
      'Tottenham Hotspur',
      'Basaksehir FK',
      'FC Metz',
      'Manchester City',
      'Real Madryt',
      'Crystal Palace',
    ],
  },
  {
    id: 581,
    name: 'Michael Essien',
    country: 'Ghana',
    clubs: ['Chelsea FC', 'Olympique Lyon', 'Real Madryt', 'AC Milan', 'Panathinaikos Ateny'],
  },
  {
    id: 582,
    name: 'Shinji Kagawa',
    country: 'Japonia',
    clubs: ['Borussia Dortmund', 'Manchester United', 'Real Zaragoza', 'Besiktas JK', 'PAOK Saloniki'],
  },
  {
    id: 583,
    name: 'Daniel Sturridge',
    country: 'Anglia',
    clubs: ['FC Liverpool', 'Chelsea FC', 'Manchester City', 'Trabzonspor', 'Bolton Wanderers', 'West Bromwich Albion'],
  },
  {
    id: 584,
    name: 'Xherdan Shaqiri',
    country: 'Szwajcaria',
    clubs: [
      'FC Basel 1893',
      'Stoke City',
      'Bayern Monachium',
      'FC Liverpool',
      'Chicago Fire FC',
      'Inter Mediolan',
      'Olympique Lyon',
    ],
  },
  {
    id: 585,
    name: 'Mateo Kovacic',
    country: 'Chorwacja',
    clubs: ['Chelsea FC', 'Real Madryt', 'Inter Mediolan', 'Dinamo Zagrzeb'],
  },
  {
    id: 586,
    name: 'Michael Owen',
    country: 'Anglia',
    clubs: ['FC Liverpool', 'Newcastle United', 'Manchester United', 'Real Madryt', 'Stoke City'],
  },
  {
    id: 587,
    name: 'Patrick Vieira',
    country: 'Francja',
    clubs: ['FC Arsenal', 'Inter Mediolan', 'Manchester City', 'Juventus Turyn', 'AC Milan'],
  },
  {
    id: 588,
    name: 'Milan Baros',
    country: 'Czechy',
    clubs: ['FC Banik Ostrava', 'Galatasaray', 'FC Liverpool', 'Aston Villa', 'Olympique Lyon', 'Antalyaspor'],
  },
  {
    id: 589,
    name: 'Luca Toni',
    country: 'Włochy',
    clubs: [
      'Hellas Verona',
      'AC Fiorentina',
      'Bayern Monachium',
      'Palermo FC',
      'Brescia Calcio',
      'Genoa CFC',
      'AS Roma',
      'Juventus Turyn',
      'FC Empoli',
    ],
  },
  {
    id: 590,
    name: 'David Villa',
    country: 'Hiszpania',
    clubs: [
      'Valencia CF',
      'New York City FC',
      'FC Barcelona',
      'Real Zaragoza',
      'Atlético Madryt',
      'Sporting Gijón',
      'Vissel Kobe',
    ],
  },
  {
    id: 591,
    name: 'Christian Vieri',
    country: 'Włochy',
    clubs: [
      'Inter Mediolan',
      'AC Fiorentina',
      'Atalanta BC',
      'Juventus Turyn',
      'Atlético Madryt',
      'Venezia FC',
      'Lazio Rzym',
      'AC Milan',
      'AS Monaco',
      'Torino FC',
    ],
  },
  {
    id: 592,
    name: 'Hernán Crespo',
    country: 'Argentyna',
    clubs: ['Parma Calcio 1913', 'Inter Mediolan', 'Chelsea FC', 'Lazio Rzym', 'AC Milan', 'Genoa CFC'],
  },
  {
    id: 593,
    name: 'Rio Ferdinand',
    country: 'Anglia',
    clubs: ['Manchester United', 'West Ham United', 'Leeds United', 'Queens Park Rangers'],
  },
  {
    id: 594,
    name: 'David Trézéguet',
    country: 'Francja',
    clubs: ['Juventus Turyn', 'AS Monaco', 'CA River Plate'],
  },
  {
    id: 595,
    name: 'Tomas Rosicky',
    country: 'Czechy',
    clubs: ['FC Arsenal', 'Borussia Dortmund', 'AC Sparta Praga'],
  },
  {
    id: 596,
    name: 'Claude Makélélé',
    country: 'Francja',
    clubs: ['Chelsea FC', 'FC Nantes', 'Real Madryt', 'FC Paris Saint-Germain', 'Celta de Vigo', 'Olympique Marseille'],
  },
  {
    id: 597,
    name: 'Fabien Barthez',
    country: 'Francja',
    clubs: ['Olympique Marseille', 'AS Monaco', 'Manchester United', 'FC Toulouse', 'FC Nantes'],
  },
  {
    id: 598,
    name: 'Dino Baggio',
    country: 'Włochy',
    clubs: ['Parma Calcio 1913', 'Juventus Turyn', 'Lazio Rzym', 'Inter Mediolan', 'Torino FC', 'Blackburn Rovers'],
  },
  {
    id: 599,
    name: 'Tiemoué Bakayoko',
    country: 'Francja',
    clubs: ['AS Monaco', 'AC Milan', 'SSC Napoli', 'Chelsea FC', 'Stade Rennais FC'],
  },
  {
    id: 600,
    name: 'Yannick Carrasco',
    country: 'Belgia',
    clubs: ['Atlético Madryt', 'AS Monaco'],
  },
  {
    id: 601,
    name: 'Keita Baldé',
    country: 'Senegal',
    clubs: ['Lazio Rzym', 'AS Monaco', 'Inter Mediolan', 'Cagliari Calcio', 'UC Sampdoria', 'Spartak Moskwa'],
  },
  {
    id: 602,
    name: 'Douglas Costa',
    country: 'Brazylia',
    clubs: ['Szachtar Donieck', 'Juventus Turyn', 'Bayern Monachium', 'Los Angeles Galaxy'],
  },
  {
    id: 603,
    name: 'Manuel Akanji',
    country: 'Szwajcaria',
    clubs: ['Borussia Dortmund', 'FC Basel 1893', 'Manchester City'],
  },
  {
    id: 604,
    name: 'Mario Gómez',
    country: 'Niemcy',
    clubs: ['VfB Stuttgart', 'Bayern Monachium', 'VfL Wolfsburg', 'AC Fiorentina', 'Besiktas JK'],
  },
  {
    id: 605,
    name: 'Mohamed Salah',
    country: 'Egipt',
    clubs: ['FC Liverpool', 'AS Roma', 'FC Basel 1893', 'AC Fiorentina', 'Chelsea FC'],
  },
  {
    id: 606,
    name: 'Alexandre Pato',
    country: 'Brazylia',
    clubs: ['AC Milan', 'FC Sao Paulo', 'Villarreal CF', 'Chelsea FC'],
  },
  {
    id: 607,
    name: 'Iago Aspas',
    country: 'Hiszpania',
    clubs: ['Celta de Vigo', 'Sevilla FC', 'FC Liverpool'],
  },
  {
    id: 608,
    name: 'Dejan Lovren',
    country: 'Chorwacja',
    clubs: ['FC Liverpool', 'Olympique Lyon', 'Zenit Petersburg', 'GNK Dinamo Zagrzeb', 'FC Southampton'],
  },
  {
    id: 609,
    name: 'Jorginho',
    country: 'Włochy',
    clubs: ['Chelsea FC', 'SSC Napoli', 'Hellas Veronag', 'FC Arsenal'],
  },
  {
    id: 610,
    name: 'Pepe Reina',
    country: 'Hiszpania',
    clubs: [
      'FC Liverpool',
      'SSC Napoli',
      'Villarreal CF',
      'Lazio Rzym',
      'FC Barcelona',
      'AC Milan',
      'Aston Villa',
      'Bayern Monachium',
    ],
  },
  {
    id: 611,
    name: 'Gervinho',
    country: 'Wybrzeże Kości Słoniowej',
    clubs: ['LOSC Lille', 'Parma Calcio 1913', 'AS Roma', 'FC Arsenal', 'Trabzonspor', 'Aris Saloniki'],
  },
  {
    id: 612,
    name: 'Kevin De Bruyne',
    country: 'Belgia',
    clubs: ['Manchester City', 'KRC Genk', 'VfL Wolfsburg', 'SV Werder Bremen', 'Chelsea FC'],
  },
  {
    id: 613,
    name: 'Julian Draxler',
    country: 'Niemcy',
    clubs: ['FC Paris Saint-Germain', 'FC Schalke 04', 'VfL Wolfsburg', 'Benfika Lizbona'],
  },
  {
    id: 614,
    name: 'Aleix Vidal',
    country: 'Hiszpania',
    clubs: ['Sevilla FC', 'RCD Espanyol', 'FC Barcelona', 'Deportivo Alavés'],
  },
  {
    id: 615,
    name: 'Mateusz Winciersz',
    country: 'Polska',
    clubs: ['Ruch Chorzów', 'Skra Częstochowa', 'Piast Gliwice'],
  },
  {
    id: 616,
    name: 'Filip Karbowy',
    country: 'Polska',
    clubs: [
      'Zagłębie Sosnowiec',
      'Chojniczanka Chojnice',
      'Wigry Suwałki',
      'Górnik Łęczna',
      'Miedź Legnica',
      'Piast Gliwice',
    ],
  },
  {
    id: 617,
    name: 'Sergiy Krykun',
    country: 'Ukraina',
    clubs: ['Górnik Łęczna', 'Garbarnia Kraków', 'Resovia Rzeszów', 'Podlasie Biała Podlaska', 'Piast Gliwice'],
  },
  {
    id: 618,
    name: 'Tiago Alves',
    country: 'Portugalia',
    clubs: ['Montedio Yamagata', 'Varzim SC', 'Olimpia Grudziądz', 'Piast Gliwice'],
  },
  {
    id: 619,
    name: 'Nikola Stojiljkovic',
    country: 'Serbia',
    clubs: ['SC Braga', 'Crvena Zvezda Belgrad', 'RCD Mallorca', 'Piast Gliwice'],
  },
  {
    id: 620,
    name: 'Dominik Steczyk',
    country: 'Polska',
    clubs: ['Hallescher FC', 'Stal Mielec', 'Ruch Chorzów', 'Piast Gliwice'],
  },
  {
    id: 621,
    name: 'Arkadiusz Pyrka',
    country: 'Polska',
    clubs: ['Znicz Pruszków', 'Piast Gliwice'],
  },
  {
    id: 622,
    name: 'Denis Gojko',
    country: 'Polska',
    clubs: ['Wigry Suwałki', 'Stal Mielec', 'Olimpia Grudziądz', 'Skra Częstochowa', 'Piast Gliwice'],
  },
  {
    id: 623,
    name: 'Aleksander Jagiello',
    country: 'Polska',
    clubs: [
      'Znicz Pruszków',
      'Podbeskidzie Bielsko-Biała',
      'Arka Gdynia',
      'Ząbkovia Ząbki',
      'Górnik Łęczna',
      'Chojniczanka Chojnice',
      'Lechia Gdańsk',
      'Piast Gliwice',
    ],
  },
  {
    id: 624,
    name: 'Bartosz Walencik',
    country: 'Polska',
    clubs: [
      'Siarka Tarnobrzeg',
      'KKS 1925 Kalisz',
      'Legionovia Legionowo',
      'Stomil Olsztyn',
      'Motor Lublin',
      'Górnik Łęczna',
      'Bytovia Bytów',
      'Piast Gliwice',
    ],
  },
  {
    id: 625,
    name: 'Pawel Tomczyk',
    country: 'Polska',
    clubs: [
      'Lech Poznań',
      'Podbeskidzie Bielsko-Biała',
      'Widzew Łódź',
      'Polonia Warszawa',
      'Stal Mielec',
      'CS Mioveni',
      'Piast Gliwice',
    ],
  },
  {
    id: 626,
    name: 'Michal Papadopulos',
    country: 'Czechy',
    clubs: [
      'Zagłębie Lubin',
      'MFK Karvina',
      'Piast Gliwice',
      'FC Banik Ostrava',
      'SC Heerenveen',
      'FK Mlada Boleslav',
      'Korona Kielce',
      'Bayer 04 Leverkusen',
      'FK Rostov',
      'FC Arsenal',
    ],
  },
  {
    id: 627,
    name: 'Karol Stanek',
    country: 'Polska',
    clubs: ['Gwarek Tarnowskie Góry', 'Widzew Łódź', 'Piast Gliwice', 'GKS Bełchatów'],
  },
  {
    id: 628,
    name: 'Stojan Vranjes',
    country: 'Bośnia i Hercegowina',
    clubs: ['Lechia Gdańsk', 'Legia Warszawa', 'Piast Gliwice', 'CFR Cluj'],
  },
  {
    id: 629,
    name: 'Josip Barisic',
    country: 'Chorwacja',
    clubs: ['Arka Gdynia', 'Stal Mielec', 'Piast Gliwice', 'Zawisza Bydgoszcz'],
  },
  {
    id: 630,
    name: 'Hebert',
    country: 'Brazylia',
    clubs: ['JEF United Chiba', 'SC Braga', 'Piast Gliwice', 'Wisła Kraków'],
  },
  {
    id: 631,
    name: 'Kornel Osyra',
    country: 'Polska',
    clubs: [
      'Miedź Legnica',
      'Podbeskidzie Bielsko-Biała',
      'Piast Gliwice',
      'Sandecja Nowy Sącz',
      'Bruk-Bet Termalica Nieciecza',
    ],
  },
  {
    id: 632,
    name: 'Michal Maslowski',
    country: 'Polska',
    clubs: ['Zawisza Bydgoszcz', 'HNK Gorica', 'Zagłębie Sosnowiec', 'GKS Katowice', 'Legia Warszawa', 'Piast Gliwice'],
  },
  {
    id: 633,
    name: 'Adrian Klepczynski',
    country: 'Polska',
    clubs: ['Widzew Łódź', 'MKS Kluczbork', 'Raków Częstochowa', 'GKS Bełchatów', 'Polonia Bytom', 'Piast Gliwice'],
  },
  {
    id: 634,
    name: 'Lukasz Hanzel',
    country: 'Polska',
    clubs: [
      'Zagłębie Lubin',
      'LKS Goczałkowice-Zdrój',
      'Podbeskidzie Bielsko-Biała',
      'Ruch Chorzów',
      'Wigry Suwałki',
      'Piast Gliwice',
    ],
  },
  {
    id: 635,
    name: 'Tomasz Podgorski',
    country: 'Polska',
    clubs: ['Podbeskidzie Bielsko-Biała', 'Ruch Chorzów', 'Piast Gliwice'],
  },
  {
    id: 636,
    name: 'Pawel Lisowski',
    country: 'Polska',
    clubs: [
      'GKS Tychy',
      'Kotwica Kołobrzeg',
      'Bytovia Bytów',
      'Stilon Gorzów Wielkopolski',
      'Ruch Chorzów',
      'Błękitni Stargard',
      'Piast Gliwice',
    ],
  },
  {
    id: 637,
    name: 'Pawel Oleksy',
    country: 'Polska',
    clubs: [
      'Ruch Chorzów',
      'Stal Rzeszów',
      'Podbeskidzie Bielsko-Biała',
      'Zawisza Bydgoszcz',
      'Piast Gliwice',
      'Zagłębie Lubin',
      'Chrobry Głogów',
      'Górnik Polkowice',
      'Arka Gdynia',
      'Bruk-Bet Termalica Nieciecza',
    ],
  },
  {
    id: 638,
    name: 'Jakub Biskup',
    country: 'Polska',
    clubs: [
      'Bałtyk Gdynia',
      'ŁKS Łódź',
      'Odra Wodzisław Śląski',
      'Chojniczanka Chojnice',
      'Piast Gliwice',
      'Bruk-Bet Termalica Nieciecza',
    ],
  },
]
